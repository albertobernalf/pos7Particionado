...
#from zeep.wsse.utils import WSE_NS
from zeep.ns import WSSE
...
class EnviarFacturaDianView(APIView):
    def post(self, request):
      ...
      try:
            ...
            XML_PRIVATE = "c:\\DIAN\\llave_privada.pem"
            ...
            # 1. Configurar la firma digital
            # Zeep usará SHA-256 de forma automática si las librerías del sistema están actualizadas
            # 1. Instancia el objeto completamente vacío (sin argumentos)
            signature = Signature()

            # 2. Configura cada propiedad de forma individual
            signature.key_file = XML_PRIVATE
            signature.certfile = CERTIFICADO_PEM
            signature.algorithm = 'sha256'  # O el algoritmo que requiera tu contraparte

            # CORRECTO (Opción A): Buscar un método "from_..."
            #signature = Signature.from_private_key(mi_clave)

            #signature = Signature.from_private_key_file(XML_PRIVATE)

            # CORRECTO: Instanciar vacío y asignar después
            #firma = Signature()
            #firma.algorithm = 'RS256'


            # 2. Pasa la clave privada cuando vas a firmar los datos
            #signature = firma.sign(CERTIFICADO_PEM, private_key=private_key)


            # 2. Inicializar el cliente SOAP con el parámetro wsse
            client = Client(wsdl='https://dian.gov.co',wsse=signature)

            # 3. Forzar la inclusión del Timestamp en el encabezado WS-Security
            # La DIAN rechaza firmas que no tengan un Timestamp válido adjunto
            wsse_element = client.wsse.create_x509_auth()
            timestamp = etree.Element(f'{{{WSE_NS}}}Timestamp')
            # Zeep se encarga de llenar el tiempo actual automáticamente al enviar
            wsse_element.append(timestamp) 

            # 4. Enviar el TestSet de forma limpia
            response = client.service.SendTestSetAsync(
