...
def FacturarCuenta(request):
	...
	# Aqui creo el registro en cartera
  	...
  	## FIN GENERAR XML CON TAGS
        ...
        #with open(nombre_archivoXml, "wb") as f:
        #       f.write(xml_final)
	...
	## Aqui guardo todos los datos de facturacion electronica en las tablas
        ...
        detalle = 'UPDATE facturacion_facturacion SET "cufeValor" = ' + "'" + str(cufe) + "'," + '"cufeDefinitivo" = ' + "'" + str(cufe) + "'," + '"codigoQr" = ' + "'" + str(qr) + "'," + '"rutaQr" = ' + "'" + str(nombre_archivoQr) + "',"  + '"rutaXml" = ' + "'" + str(nombre_archivoXml) + "'," + '"rutaJson" = ' + "'" + str(nombre_archivoJson)  + "'," + '"rutaPdf" = ' + "'" + str(nombre_archivoPdf) + "' WHERE id = " + str(facturacionId)
...
class EnviarFacturaDianView(APIView):
    def post(self, request):
      ...
      # Aquí debes implementar tu lógica para transformar los datos JSON 
      # a la estructura XML exigida por la DIAN y firmarlo digitalmente.
      #xml_bytes = self.generar_y_firmar_xml(datos_factura)
        
      # 2. CONSUMIR EL WEB SERVICE SOAP DE LA DIAN
      # URL de los Web Services de la DIAN (WSDL) - Usar el de Producción o Habilitación según corresponda
      #wsdl_url = "https://dian.gov.co"
      ...
      try:
            ...
            #header_element = client.get_element('ns0:62edb572-e4b6-42dd-a896-f4765e4ee8bb')
            #print ("Pase Cliente1 ")
            #header_value = header_element('62edb572-e4b6-42dd-a896-f4765e4ee8bb')
            ...
            roota = etree.fromstring(xml_bytes)

            print ("Voy a comenzar el proceso de Fira Electronica")
    
            # La DIAN requiere la firma estándar envelop

            # 1. Update the signature creation method
            signature_node = xmlsec.template.create(
               roota,
               c14n_method=xmlsec.Transform.EXCL_C14N, # <-- Use this instead of DSigExcC14NTransform
            ...
            #signature_node = xmlsec.template.create(
            # roota,
            #       xmlsec.constants.DSigExcC14NTransform,  xmlsec.constants.SignatureMethodRSA_SHA256
            #)
            roota.append(signature_node)
            print("Pase signature")
            ...
            print ("Voy a cargar el certificado")
    
            # Cargar el certificado (soporta formatos como .pfx, .p12, .pem)
            ctx.key = xmlsec.Key.from_file(
               #ruta_certificado, 
               #"c:\\DIAN\\ALBERTO BERNAL FERREIRA.p12",
               "c:\\DIAN\\certificado.pem",
            ...
            ctx.key.load_cert_from_file('c:\\DIAN\\ALBERTO BERNAL FERREITA.p12', xmlsec.constants.KeyDataFormatPkcs12)
            print("En teoria ya incruste el Certificado en el XML")
            # 5. Ejecutar la firma sobre el nodo creado
            ctx.sign(signature_node)

            # 6. Serializar el documento a una cadena de texto (bytes)
            xml_firmado_bytes = etree.tostring(roota, encoding='utf-8', xml_declaration=True)
    

            #################################################################################################
            ## fin pasos python firmado
            ...
            archivo_base64 = base64.b64encode(xml_firmado_bytes).decode('utf-8')

            # 3. Preparar los datos que exige el método 'SendBillSync' de la DIAN
            datos_envio = {
                 "fileName": "fv_SETG_980000001.xml", # El nombre debe coincidir con el XML
            ...
            print(" Ops Entre con error = ", str(e))
            return Response({
