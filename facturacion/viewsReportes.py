...
from sitios.models import Dependencias, SedesClinica
...
from facturacion.models import ConveniosPacienteIngresos, Facturacion , TiposSuministro
...
class PDFFacturacion(FPDF):
    ...
    def header(self):
        ...
        if (self.flag=='F'):
            print("voy por codigoqr", factura.rutaQr)
     
            if (factura.rutaQr != None):
                  print("Entre codigo Qr1" , factura.rutaQr)
                  self.image(factura.rutaQr, 185, 5, 20, 20)
                  print("Entre codigo Qr2")
            else:
                  print("Entre pass")
                  pass
        ...
        self.ln(3)
        # Logo
        self.image('C:/EntornosPython/Pos7Particionado/vulner/static/img/ClinicaAlb.png', 7, 5, 30, 17)
        # Arial bold 15
        self.set_font('helvetica', 'B', 8)
        self.ln(3)
        ...
        if (self.flag=='F'):
            ...
            self.cell(25, 10, str(factura.factura), 0, 0, 'L')
        else:

            self.cell(60, 10, 'PREFATURA:', 0, 0, 'L')
            self.cell(25, 10, str(factura.id), 0, 0, 'L')
        print("AQUI VOY 01")
        ...
        print("AQUI VOY self.factura=", self.factura)
...
def ImprimirFactura(request):
    ...
    print ("A FLOTE_01")
    tiposSuminitroMaterialSuturaQx = TiposSuministro.objects.get(nombre='MATERIAL DE SUTURA Y CURACION QX')
    print ("A FLOTE_02")
    honorarioMaterialSuturaQx = TiposHonorarios.objects.get(nombre='MATERIALES DE SUTURA Y CURACION QX')
    honorarioMaterialQx = TiposHonorarios.objects.get(nombre='MATERIAL QX')
    print ("A FLOTE_03")
    tiposSuminitroInsumosMedicos = TiposSuministro.objects.get(nombre='INSUMOS MEDICOS')
    tiposSuminitroMaterialSuturaNoQx = TiposSuministro.objects.get(nombre='MATERIAL DE SUTURA Y CURACION NOQX')
    tiposSuministroDispositivos = TiposSuministro.objects.get(nombre='DISPOSITIVOS MEDICOS')
    tiposSuministroMaterialQx = TiposSuministro.objects.get(nombre='MATERIAL QX')
    print ("A FLOTE_04")
    ...
    comando = 'SELECT distinct con.id, con.nombre nombreConcepto from facturacion_conceptos con  INNER JOIN facturacion_facturaciondetalle facDet  ON (facDet.concepto_id = con.id and con.nombre != ' + "'" + str('MATERIAL QX Y/O MATERIAL ESPEC') + "')" + '  INNER JOIN clinico_examenes exa ON (exa.id= facDet.examen_id) where facdet.facturacion_id = ' + "'" + str(factura) + "'" + ' union SELECT distinct con.id, con.nombre nombreConcepto from facturacion_conceptos con INNER JOIN facturacion_facturaciondetalle facDet  ON (facDet.concepto_id = con.id and con.nombre != ' + "'" + str('MATERIAL QX Y/O MATERIAL ESPEC') + "')" + ' INNER JOIN facturacion_suministros sum ON (sum.id = facDet.cums_id) where facdet.facturacion_id = ' + "'" + str(factura) + "'" + ' order by 1 asc '
    print(comando)
    curt.execute(comando)
    ...
    for id, nombreConcepto in curt.fetchall():
        ...
        miConexiony = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                       password="123456")
        ...
        if (id == conceptoInsumosMedicos.id):
            ...
            comando = 'select sum.cums cups, ' + "' '" +  ' homologado, sum.nombre  descripcion, detFac.cantidad cantidad,detFac."valorUnitario" valorUnitario,detFac."valorTotal" valorTotal FROM facturacion_facturaciondetalle detFac INNER JOIN facturacion_facturacion fac ON (fac.id=detFac.facturacion_id) INNER JOIN facturacion_tipossuministro tip on (tip.concepto_id = ' + "'" + str(conceptoInsumosMedicos.id) + "'" + 	' and tip.nombre=' + "'" + str(tiposSuminitroInsumosMedicos.nombre) + "')" + ' INNER JOIN facturacion_suministros sum on (sum.id=detFac.cums_id and sum."tipoSuministro_id" = tip.id) where detfac.facturacion_id= ' + "'" + str(factura) + "'" + ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" +  ' and detFac.cirugia_id is null ORDER BY sum.cums '
            print("entre iNSUMOS MEDICOS QUE NO SON DE CIRUGIA QX")

        if (id ==conceptoMaterialQx.id):

            comando = 'select sum.cums cups, ' + "' '" +  ' homologado, sum.nombre  descripcion, detFac.cantidad cantidad,detFac."valorUnitario" valorUnitario,detFac."valorTotal" valorTotal FROM facturacion_facturaciondetalle detFac INNER JOIN facturacion_facturacion fac ON (fac.id=detFac.facturacion_id) INNER JOIN facturacion_tipossuministro tip on (tip.concepto_id = ' + "'" + str(conceptoInsumosMedicos.id) + "'" + 	' and tip.nombre IN (' + "'" + str(tiposSuministroDispositivos.nombre) + "','" + str(tiposSuministroMaterialQx.nombre) + "'))" + ' INNER JOIN facturacion_suministros sum on (sum.id=detFac.cums_id and sum."tipoSuministro_id" = tip.id) where detfac.facturacion_id= ' + "'" + str(factura) + "'" + ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" +  ' and detFac.cirugia_id is null ORDER BY sum.cums '
 
            print("entre Material Qx QUE NO SON DE CIRUGIA")



        if (id == conceptoMedicamentos.id):

            comando = 'select sum.cums cups,case when detFac."codigoHomologado" is not null then detFac."codigoHomologado" else tarSum."codigoHomologado" end homologado, sum.nombre  descripcion, detFac.cantidad cantidad, 	detFac."valorUnitario" valorUnitario, detFac."valorTotal" valorTotal FROM facturacion_facturaciondetalle detFac INNER JOIN facturacion_facturacion fac ON (fac.id=detFac.facturacion_id) INNER JOIN facturacion_suministros sum on (sum.id=detFac.cums_id) INNER JOIN contratacion_convenios conv ON (conv.id=fac.convenio_id)  LEFT JOIN tarifarios_tarifariosdescripcion tarDesc ON (tarDesc.id=conv."tarifariosDescripcionSum_id") LEFT JOIN tarifarios_tarifariossuministros tarSum ON (tarSum."tiposTarifa_id"=tarDesc."tiposTarifa_id" AND tarSum."codigoCum_id" = detFac.cums_id ) where detfac.facturacion_id= ' + "'" + str(factura) + "'" + ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" +   ' AND detFac.concepto_id = ' + "'" + str(id) + "'" + ' and detFac.cirugia_id is null  ORDER BY sum.cums'
        ...
        if (id != conceptoMedicamentos.id and id != conceptoProcedimientosQx.id and id != conceptoInsumosMedicos.id and id != conceptoMaterialQx.id):
            ...
            comando = 'select exa."codigoCups" cups,case when detFac."codigoHomologado" is not null then detFac."codigoHomologado" else tarProc."codigoHomologado" end homologado, exa.nombre  descripcion, detFac.cantidad cantidad, detFac."valorUnitario" valorUnitario, detFac."valorTotal" valorTotal FROM facturacion_facturaciondetalle detFac INNER JOIN facturacion_facturacion fac ON (fac.id=detFac.facturacion_id) INNER JOIN clinico_examenes exa on (exa.id=detFac.examen_id) INNER JOIN contratacion_convenios conv ON (conv.id=fac.convenio_id) LEFT JOIN tarifarios_tarifariosdescripcion tarDesc ON (tarDesc.id=conv."tarifariosDescripcionProc_id") LEFT JOIN tarifarios_tarifariosprocedimientos tarProc ON (tarProc."tiposTarifa_id"=tarDesc."tiposTarifa_id" AND tarProc."codigoCups_id" = detFac.examen_id ) where detfac.facturacion_id= ' + "'" + str(factura) + "'" +  ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" + " AND detFac.concepto_id = " +"'" + str(id) + "'" + ' ORDER BY exa."codigoCups"'
    ...
    #prefijoDian = SedesClinica.objects.get(id=sede)
    #print("prefijoDian = ", prefijoDian.prefijoDian)
    #numeroFacturaPrefijo =  prefijoDian.prefijoDian + str(facturacionId)
    numeroFacturaPrefijo =  facturaPaciente.factura
    ...
    nombre_archivoPdf = nombre_carpeta + caracter_especial + str(numeroFacturaPrefijo.strip()) + '.pdf'
    ...
    archivo = nombre_archivoPdf
    print("nombre_archivoPdf =", nombre_archivoPdf)
    ...
    try:
       ...
       pdf.output(nombre_archivoPdf, 'F')
       buff = BytesIO()
       buff.name = nombre_archivoPdf
       # Genera el archivo el el servidor

        # Abrir el archivo PDF y leerlo
       with open(nombre_archivoPdf, 'rb') as f:
           pdf_data = f.read()
       ...
       #with open(nombre_archivoPdf, 'w' , encoding='utf-8') as archivo:
       #    # Escribir el texto en el archivo
       #    file = open(nombre_archivoPdf, "w")
       #    print("Archivo pdf  GUARDADO= " , BytesIO())
       #    file.writelines(BytesIO())
       #    file.close()
    ...
    print ("Lo voy a abrir directamente")

    return JsonResponse({'success': True, 'Mensajes': 'Factura impresa!'})
...
def ImprimirLiquidacion(request):
    ...
    comando = 'SELECT distinct con.id, con.nombre nombreConcepto from facturacion_conceptos con  INNER JOIN facturacion_liquidaciondetalle facDet  ON (facDet.concepto_id = con.id and con.nombre != ' + "'" + str('MATERIAL QX Y/O MATERIAL ESPEC') + "')" + '  INNER JOIN clinico_examenes exa ON (exa.id= facDet.examen_id) where facdet.liquidacion_id = ' + "'" + str(factura) + "'" + ' union SELECT distinct con.id, con.nombre nombreConcepto from facturacion_conceptos con INNER JOIN facturacion_liquidaciondetalle facDet  ON (facDet.concepto_id = con.id and con.nombre != ' + "'" + str('MATERIAL QX Y/O MATERIAL ESPEC') + "')" + ' INNER JOIN facturacion_suministros sum ON (sum.id = facDet.cums_id) where facdet.liquidacion_id = ' + "'" + str(factura) + "'" + ' order by 1 asc '
    ...
    for id, nombreConcepto in curt.fetchall():
        ...
        if (id == conceptoInsumosMedicos.id):
            ...
            comando = 'select sum.cums cups, ' + "' '" +  ' homologado, sum.nombre  descripcion, detFac.cantidad cantidad,detFac."valorUnitario" valorUnitario,detFac."valorTotal" valorTotal FROM facturacion_liquidaciondetalle detFac INNER JOIN facturacion_liquidacion fac ON (fac.id=detFac.liquidacion_id) INNER JOIN facturacion_tipossuministro tip on (tip.concepto_id = ' + "'" + str(conceptoInsumosMedicos.id) + "'" + 	' and tip.nombre=' + "'" + str(tiposSuminitroInsumosMedicos.nombre) + "')" + ' INNER JOIN facturacion_suministros sum on (sum.id=detFac.cums_id and sum."tipoSuministro_id" = tip.id) where detfac.liquidacion_id= ' + "'" + str(factura) + "'" + ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" +  ' and detFac.cirugia_id is null ORDER BY sum.cums '
            print("entre iNSUMOS MEDICOS QUE NO SON DE CIRUGIA QX")


        if (id ==conceptoMaterialQx.id):

            comando = 'select sum.cums cups, ' + "' '" +  ' homologado, sum.nombre  descripcion, detFac.cantidad cantidad,detFac."valorUnitario" valorUnitario,detFac."valorTotal" valorTotal FROM facturacion_liquidaciondetalle detFac INNER JOIN facturacion_liquidacion fac ON (fac.id=detFac.liquidacion_id) INNER JOIN facturacion_tipossuministro tip on (tip.concepto_id = ' + "'" + str(conceptoInsumosMedicos.id) + "'" + 	' and tip.nombre IN (' + "'" + str(tiposSuministroDispositivos.nombre) + "','" + str(tiposSuministroMaterialQx.nombre) + "'))" + ' INNER JOIN facturacion_suministros sum on (sum.id=detFac.cums_id and sum."tipoSuministro_id" = tip.id) where detfac.liquidacion_id= ' + "'" + str(factura) + "'" + ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" +  ' and detFac.cirugia_id is null ORDER BY sum.cums '
 
            print("entre Material Qx QUE NO SON DE CIRUGIA")

        if (id == conceptoMedicamentos.id):

            comando = 'select sum.cums cups,case when detFac."codigoHomologado" is not null then detFac."codigoHomologado" else tarSum."codigoHomologado" end homologado, sum.nombre  descripcion, detFac.cantidad cantidad, 	detFac."valorUnitario" valorUnitario, detFac."valorTotal" valorTotal FROM facturacion_liquidaciondetalle detFac INNER JOIN facturacion_liquidacion fac ON (fac.id=detFac.liquidacion_id) INNER JOIN facturacion_suministros sum on (sum.id=detFac.cums_id) INNER JOIN contratacion_convenios conv ON (conv.id=fac.convenio_id)  LEFT JOIN tarifarios_tarifariosdescripcion tarDesc ON (tarDesc.id=conv."tarifariosDescripcionSum_id") LEFT JOIN tarifarios_tarifariossuministros tarSum ON (tarSum."tiposTarifa_id"=tarDesc."tiposTarifa_id" AND tarSum."codigoCum_id" = detFac.cums_id ) where detfac.liquidacion_id= ' + "'" + str(factura) + "'" + ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" +   ' AND detFac.concepto_id = ' + "'" + str(id) + "'" + ' and detFac.cirugia_id is null  ORDER BY sum.cums'
            print("entre Medicamentos")

        if (id != conceptoMedicamentos.id and id != conceptoProcedimientosQx.id and id != conceptoInsumosMedicos.id and id != conceptoMaterialQx.id):

            comando = 'select exa."codigoCups" cups,case when detFac."codigoHomologado" is not null then detFac."codigoHomologado" else tarProc."codigoHomologado" end homologado, exa.nombre  descripcion, detFac.cantidad cantidad, detFac."valorUnitario" valorUnitario, detFac."valorTotal" valorTotal FROM facturacion_liquidaciondetalle detFac INNER JOIN facturacion_liquidacion fac ON (fac.id=detFac.liquidacion_id) INNER JOIN clinico_examenes exa on (exa.id=detFac.examen_id) INNER JOIN contratacion_convenios conv ON (conv.id=fac.convenio_id) LEFT JOIN tarifarios_tarifariosdescripcion tarDesc ON (tarDesc.id=conv."tarifariosDescripcionProc_id") LEFT JOIN tarifarios_tarifariosprocedimientos tarProc ON (tarProc."tiposTarifa_id"=tarDesc."tiposTarifa_id" AND tarProc."codigoCups_id" = detFac.examen_id ) where detfac.liquidacion_id= ' + "'" + str(factura) + "'" +  ' AND (detfac.anulado =' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" + " AND detFac.concepto_id = " +"'" + str(id) + "'" + ' ORDER BY exa."codigoCups"'
            print ("entre proced Normales")
        ...
        if (id != conceptoProcedimientosQx.id):
	        ...
	        ## Por aqui es un Procedimiento Qx Detalle
            print ("Entre a liquidar cirugias con honorarios =" , liquidaHonorarios)

            if (liquidaHonorarios=='ISS'):
                liquidacionTotalesProc = []
                print("Entre a liquidar ISS cirugias ")
                comando = 'select exa.id idCups, exa."codigoCups" cups,tarProc."codigoHomologado" homolo , exa.nombre  descripcion, 1 cantidad,sum(detFac."valorUnitario") valorUnitario, sum(detFac."valorTotal") valorTotal FROM facturacion_liquidaciondetalle detFac INNER JOIN facturacion_liquidacion fac ON (fac.id=detFac.liquidacion_id) INNER JOIN clinico_examenes exa on (exa.id=detFac.examen_id) INNER JOIN contratacion_convenios conv ON (conv.id=fac.convenio_id) LEFT JOIN tarifarios_tarifariosdescripcion tarDesc ON (tarDesc.id=conv."tarifariosDescripcionProc_id") LEFT JOIN tarifarios_tarifariosprocedimientos tarProc ON (tarProc."tiposTarifa_id"=tarDesc."tiposTarifa_id" AND tarProc."codigoCups_id" = detFac.examen_id ) where detfac.liquidacion_id= ' + "'" + str(factura) + "'" +  ' AND (detfac.anulado = ' + "'" + str('N') + "'" + ' or detfac.anulado=' + "'" + str('R') + "')" + ' AND detFac.concepto_id = ' + "'" + str(salvoConcepto) + "'" + ' and detFac."tipoHonorario_id" is not null  and cirugia_id is not null group by exa.id,  exa."codigoCups", tarProc."codigoHomologado",exa.nombre ORDER BY 2,1'
                print("COMANDO =", comando)
                cury.execute(comando)
...
def ImprimirPazYSalvo(request):
    ...
    return JsonResponse({'success': True, 'Mensajes': 'Paz y salvo impreso!'})
