...
class PDF(FPDF):
    def __init__(self, tipoDocId,documentoId, consec, esTriage, flag, *args, **kwargs):
	...
	## FIN CURSOR
        ...
        self.ln(3)
        if (float(self.flag)==0):
            print("entre HC")
            self.cell(195, 12, 'HISTORIA CLINICA', 0, 0, 'C')
        if (float(self.flag)==1):
            self.cell(195, 12, 'EPICRISIS', 0, 0, 'C')
        if (float(self.flag)==2):
            self.cell(195, 12, 'HISTORIA CLINICA PARCIAL', 0, 0, 'C')
        self.ln(5)
...
def ImprimirHistoriaClinica(request):
    ...
    desdeFolio = request.POST["desdeFolio"]
    hastaFolio = request.POST["hastaFolio"]
    flag=flag.strip()

    print("flag =", flag)
    print("desdeFolio =", desdeFolio)
    print("hastaFolio =", hastaFolio)  
    ...
    pdf = PDF(tipoDocId,documentoId, consec, estriage, flag)
    ...
    totalFolios = 20
    print("A testear flag")

    uno=1

    if (float(flag) == float(uno)):
       print("Entre flag 1")
       minimo = Historia.objects.filter(tipoDoc_id=tipoDocId,documento_id=documentoId,consecAdmision=consec).aggregate(minFolio=Min('folio'))
       print("minimo=" , minimo)
       maximo = Historia.objects.filter(tipoDoc_id=tipoDocId,documento_id=documentoId,consecAdmision=consec).aggregate(maxFolio=Max('folio'))
       print("maximo=" , maximo)
       desdeFolio = minimo['minFolio']
       hastaFolio = maximo['maxFolio']
       print("maximo vale = ", maximo)

    # El propgrama debe preguntar desde que Folio hasta cual Y/O desde que fecha y hasta cual fecha

    # Cursor recorre Folios

    print("desdeFolio Final = ", desdeFolio)
    print("hastaFolio Final = ", hastaFolio)

    miConexiont = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                   password="123456")
    curt = miConexiont.cursor()

    comando = 'select  h.id HistoriaId, h.folio folioId, h.fecha fechaFolio, tip.nombre tipoFolio from clinico_historia h left join clinico_tiposfolio tip ON (tip.id = h."tiposFolio_id" ) where h.documento_id = ' + "'" + str(documentoId) + "' AND h.folio >= " + "'" + str(desdeFolio) + "' AND h.folio <= "  + "'" + str(hastaFolio) + "'" + ' order by h.id'
    curt.execute(comando)
    #print(comando)
    ...
    for i in range(0, len(folios)):
        ...
        pdf.cell(1, 1,  '___________________________________________________________________________________________________________________________',  0, 0, 'L')
        ...
        for l in range(0, len(historia)):
            ...
            miConexionh = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                           password="123456")
            ...
            miConexionr = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                           password="123456")
            ...
            miConexiond = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                           password="123456")
            ...
            miConexione = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                           password="123456")
            ...
            for l in range(0, len(laboratorios)):
                ...
                miConexionp = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                               password="123456")
            ...
            for l in range(0, len(radiologia)):
                ...
                miConexiona = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                               password="123456")
            ...
            for l in range(0, len(terapias)):
                ...
                miConexionb = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                               password="123456")
            ...
            miConexionc = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                           password="123456")
            ...
            miConexioni = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",
                                           password="123456")
...
def ImprimirOrdenIncapacidad(ingresoId2, historiaId,convenioId , tipoAdmision):
    ...
    miConexioni = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432", user="postgres",    password="123456")
    curi = miConexioni.cursor()
