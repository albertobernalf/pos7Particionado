...
from facturacion.models import ConveniosPacienteIngresos, Liquidacion, LiquidacionDetalle, Facturacion, FacturacionDetalle, TiposSuministro, Conceptos
...
from rips.models import RipsTransaccion, RipsUsuarios, RipsEnvios, RipsDetalle, RipsTiposNotas, RipsOtrosServicios
import pickle
import io
...
def load_dataDetalleRips(request, data):
    ...
    detalle = 'SELECT rips_ripsDetalle.id,  "numeroFactura_id"  numeroFactura , usu.nombre nombre, glosa_id glosaId, "notaCredito_id" notaCreditoId , "cuv", "estadoPasoMinisterio","rutaJsonRespuesta", "rutaJsonFactura",  rips_ripsDetalle."fechaRegistro", rips_ripsDetalle."estadoReg", "ripsEnvios_id",  rips_ripsDetalle."usuarioRegistro_id", rips_ripsDetalle."rutaPdf", rips_ripsDetalle."rutaZip"  FROM public.rips_ripsDetalle INNER JOIN facturacion_facturacion fac on (fac.id=rips_ripsDetalle."numeroFactura_id") INNER JOIN usuarios_usuarios usu on (usu.id =fac.documento_id) WHERE "ripsEnvios_id" =  ' + "'" + str(envioRipsId) + "'"

    print(detalle)

    curx.execute(detalle)

    for id,  numeroFactura,nombre, glosaId , notaCreditoId, cuv, estadoPasoMinisterio,rutaJsonRespuesta,rutaJsonFactura,  fechaRegistro, estadoReg, ripsEnvios_id, usuarioRegistro_id, rutaPdf, rutaZip in curx.fetchall():
        detalleRips.append(
            {"model": "rips.detalleRips", "pk": id, "fields":
                {'id': id, 'numeroFactura': numeroFactura ,'nombre':nombre ,'glosaId':glosaId,  'notaCreditoId':notaCreditoId, 'cuv': cuv, 'estadoPasoMinisterio': estadoPasoMinisterio, 'rutaJsonRespuesta':rutaJsonRespuesta, 'rutaJsonFactura':rutaJsonFactura,
...
def load_dataDetalleRipsAdicionar(request, data):
    ...
    if (tipoRips == 'Glosa'):
        ...
        detalle = 'SELECT glosas.id, gloDet.factura_id factura, glosas.id glosaId, glosas."fechaRecepcion" fechaFactura, u.nombre paciente , gloDet."valorAceptado" totalFactura,' + "' '" + ' estado  FROM cartera_glosas glosas , facturacion_facturacion f , admisiones_ingresos i, usuarios_usuarios u  , cartera_glosasdetalle gloDet 	WHERE gloDet."glosa_id" = glosas.id AND  gloDet.factura_id = f.id AND i."tipoDoc_id" = f."tipoDoc_id" AND i.documento_id = f.documento_id AND i.consec = f."consecAdmision"  AND glosas."ripsEnvio_id" IS NULL AND i."tipoDoc_id" = u."tipoDoc_id" AND 	i.documento_id = u.id  and glosas."totalAceptado" >0'

    if (tipoRips == 'Nota Credito'):
        
	    detalle = 'SELECT nc.id, ncDet.factura_id factura, nc.id glosaId, nc."fechaNota" fechaFactura, u.nombre paciente , ncDet."valorNotaTotal" totalFactura,' + "' '" + ' estado  FROM cartera_notascredito nc , facturacion_facturacion f , admisiones_ingresos i, usuarios_usuarios u  , cartera_notascreditodetalle ncDet 	WHERE ncDet."notaCredito_id" = nc.id AND ncDet.factura_id = f.id AND i."tipoDoc_id" = f."tipoDoc_id" AND i.documento_id = f.documento_id AND i.consec = f."consecAdmision"  AND nc."ripsEnvio_id" IS NULL AND i."tipoDoc_id" = u."tipoDoc_id" AND 	i.documento_id = u.id  and nc."valorNotaTotal" >0'
...
def ActualizarEmpresaDetalleRips(request):
    ...
    try:
            ...
            if (tipoRips == 'Glosa'):
                ...
                print("entre glosa para ripsdetalle")
                comando1 = 'INSERT INTO RIPS_RIPSDETALLE ("numeroFactura_id","glosa_id", "estadoPasoMinisterio", "fechaRegistro", "estadoReg",	"ripsEnvios_id", "usuarioRegistro_id", estado) select gloDet.factura_id, gloDet."glosa_id",' + "'" + str('N') + "','" + str(fechaRegistro) + "','" + str(estadoReg) + "','" + str(envioRipsId) + "'," + "'" + str(username_id) + "'," + "'" + str('ELABORADA') + "'" + ' FROM cartera_glosasdetalle gloDet where gloDet.glosa_id=' + "'" + str(glosaId) + "' AND gloDet.factura_id = " + str(facturaId)
            ...
            if (tipoRips == 'Nota Credito'):
                ...
                comando1 = 'INSERT INTO RIPS_RIPSDETALLE ("numeroFactura_id","notaCredito_id", "estadoPasoMinisterio", "fechaRegistro", "estadoReg",	"ripsEnvios_id", "usuarioRegistro_id", estado) select ncDet.factura_id, ncDet."notaCredito_id",' + "'" + str('N') + "','" + str(fechaRegistro) + "','" + str(estadoReg) + "','" + str(envioRipsId) + "'," + "'" + str(username_id) + "'," + "'" + str('ELABORADA') + "'" + ' FROM cartera_notascreditodetalle ncDet where ncDet."notaCredito_id"=' + str(notaCreditoId) + " AND ncDet.factura_id = " + str(facturaId)
...
def GenerarJsonRips(request):
    ...
    try:
            ...
            print("BORRO OTROS SERVICIOS ", detalle4)
            ...
            print("detalle6 = ", detalle6)
            resultado = curx.execute(detalle6)
            print("BORRO tx ", detalle6)

            ## AQUI VENDRIA LA VALIDACION DE SI PRIMERO RECERAR GLOSA/NC

            detalle7 ='select count(*) total from cartera_notascreditodetalle creDet where creDet.factura_id in (select "numeroFactura_id" from rips_ripsdetalle ripsDet  where ripsDet."ripsEnvios_id" = '  +  str(envioRipsId) + ') and creDet.id in (select "notaCreditoDetalle_id" from cartera_notascreditodetallerips detRips where "notaCreditoDetalle_id" = creDet.id)'
            totalNc = curx.execute(detalle7)
            print("totalNc = ", totalNc)

            toca=[]

            for total in curx.fetchall():

                print("TOTAL DE PRIMERA  = ", total)

                toca.append({'total':total})

                if (tipoRips == 'Factura'):

                 print ("Entre valida si hay o no gloas-nc")
                 #detalleRips= RipsDetalle.objects.filter(numeroFactura_id=elemento  , glosa_id__isnull=False).count()

                 totalU1 = str(total)
                 totalU1 =totalU1.replace(")", " ")
                 totalU1 =totalU1.replace("(", " ")
                 totalU1 =totalU1.replace(",", " ")

                 print("TOTAL = ", totalU1)
 
                if (float(totalU1) >= 1.0):

                     mensaje = f"Entre No puedo generar RIPS de la factura porque tengo Notas Creditos Activas, {totalU1}. ¡"

                     print (mensaje)
                     return JsonResponse({'success': True, 'Mensajes': mensaje})

            detalle8 ='select count(*) total2 from cartera_glosasdetalle gloDet where gloDet.factura_id in (select "numeroFactura_id" from rips_ripsdetalle ripsDet  where ripsDet."ripsEnvios_id" = ' + str(envioRipsId) +') and gloDet.id in (select "glosasDetalle_id" from cartera_glosasdetallerips detRips where detRips."glosasDetalle_id" = gloDet.id)'
            totalGlosas = curx.execute(detalle8)

            print("totalGlosas = ", totalGlosas)

            toca1=[]

            for total2 in curx.fetchall():
                toca1.append({'total2':total2})

                if (tipoRips == 'Factura'):

                 #detalleRips2= RipsDetalle.objects.filter(numeroFactura_id=elemento, notaCredito_id__isnull=False).count()
                 #print("detalleRips2 = ", detalleRips2)

                 totalU2 = str(total2)
                 totalU2 =totalU2.replace(")", " ")
                 totalU2 =totalU2.replace("(", " ")
                 totalU2 =totalU2.replace(",", " ")

                 if (float(totalU2) >= 1.0):

                     mensaje = f"Entre No puedo generar RIPS de la factura porque tengo Glosas Activas, {totalU2}. ¡"
    ...
    try:
            ...
            if (tipoRips == 'Factura'):
                ...
                detalle =  'SELECT id, "numeroFactura_id" item , "numeroFactura_id" itemOtro, 0 itemDetalle  from rips_ripsdetalle det where det."ripsEnvios_id"  = ' + "'" + str(envioRipsId) +"'"
                curx.execute(detalle)

                for id, item, itemOtro, itemDetalle in curx.fetchall():
                    barridoFacturas.append({'id':id, 'item': item,'itemOtro':itemOtro , 'itemDetalle':itemDetalle })
            ...
            if (tipoRips == 'Glosa'):
                ...
                detalle = 'SELECT det.id, det."glosa_id" item , gloDet.factura_id itemOtro, det.id itemDetalle from rips_ripsdetalle det left join cartera_glosas glosas on (glosas."ripsEnvio_id" =det."ripsEnvios_id" and glosas.id =det."glosa_id" ) left join cartera_glosasdetalle gloDet on (gloDet."glosa_id" =glosas.id  and gloDet.factura_id = det."numeroFactura_id") where det."ripsEnvios_id"  = ' + "'" + str(envioRipsId) + "'"
            ...
            if (tipoRips == 'Nota Credito'):
                ...
                detalle = 'SELECT det.id, det."notaCredito_id" item , ncDet.factura_id itemOtro , ncDet.id itemDetalle from rips_ripsdetalle det left join cartera_notascredito nc on (nc."ripsEnvio_id" =det."ripsEnvios_id" and nc.id =det."notaCredito_id" ) left join cartera_notascreditodetalle ncDet on (ncDet."notaCredito_id" =nc.id  and ncDet.factura_id = det."numeroFactura_id") where det."ripsEnvios_id"  = ' + "'" + str(envioRipsId) + "'"
                curx.execute(detalle)
            ...
            for elementox in barridoFacturas:
                ...
                print("elementoOtro = ", elementoOtro)
                itemDetalleU = elementox['itemDetalle']
                print("itemDetalleU = ", itemDetalleU)
                traigoConsecutivo=1
                van = van +1

                facturaFev = Facturacion.objects.get(id=elemento)
                print ("facturaFEV = ", facturaFev)
                ...
                if (tipoRips == 'Glosa'):
                    ...
                    print("Entre por glosa con factura = ", int(elementoOtro))
                    print("van=", van)

                    #try:
                    #    print("voy a ensayar")
                        #facturaId = Facturacion.objects.get(pk=elementoOtro)
                    #    obj = Facturacion.objects.get(id=elementoOtro)
                    #    print("ya ensaye")

                    #except (Facturacion.DoesNotExist, ValueError, TypeError):
                    ...
                    #    print("No existe basedatos")

                    #finally:
                        # SE EJECUTA SIEMPRE: Ideal para limpieza de recursos o logs finales
                    #    print("Operación de base de datos finalizada.")
                    ...
                    comando = 'select i.id ingreso FROM admisiones_ingresos i inner join facturacion_facturacion fac on (fac."tipoDoc_id" = i."tipoDoc_id" AND fac.documento_id=i.documento_id AND fac."consecAdmision" = i.consec) where fac.id = ' + str(elementoOtro)
                    print("comando= ", comando)

                    curx.execute(comando)

                    for ingreso in curx.fetchall():
                       ingresoActual.append({'ingreso': ingreso})

                    print ("ingresoActual =" , ingresoActual)

                    for elemental in ingresoActual:  # 1. "for" recorre la lista [1]

                        print("indice = ", elemental)
                        valar =  elemental['ingreso']

                    #valor=valor.strip()
                    valarU = str(valar)
                    valarU = valarU.replace("Decimal(", ' ')
                    valarU =valarU.replace("),", ",")
                    valarU = valarU.replace(",)", " ")
                    valarU = valarU.replace("(", " ")
                    print("valarU = ", valarU)
                    ...
                    detalle =  'INSERT into rips_ripstransaccion ("numDocumentoIdObligado",  "numNota","fechaRegistro", "tipoNota_id","usuarioRegistro_id" ,"ripsEnvio_id","sedesClinica_id" ,"numFactura" ,"estadoReg") select substring(sed.nit,1,9) ,  glosas.id, now(), tipnot.id,' + "'" + str(username_id) + "'" + ' , e.id, sed.id , ' + "'" + str(elementoOtro) + "','"  + str('A') + "'" + ' from sitios_sedesclinica sed, cartera_glosas glosas, cartera_glosasdetalle gloDet, rips_ripsEnvios e  , rips_ripsdetalle det ,rips_ripstiposnotas tipnot where e.id = ' + "'" + str(envioRipsId) + "'" + ' and e."sedesClinica_id" = sed.id and glosas."ripsEnvio_id" = e.id and det."ripsEnvios_id" = e.id and e."ripsTiposNotas_id" = tipnot.id and tipnot.nombre=' + "'" + str('Glosa') + "'" + ' AND gloDet.factura_id = ' + "'" + str(elementoOtro) + "'"  + ' and glosas.id = gloDet."glosa_id" and det."glosa_id" = glosas.id and det."numeroFactura_id" = gloDet.factura_id AND gloDet.glosa_id = ' + "'" + str(elemento) + "'" + ' RETURNING id ;'
                ...
                if (tipoRips == 'Nota Credito'):
                    ...
                    detalle = 'INSERT into rips_ripstransaccion ("numDocumentoIdObligado",  "numNota","fechaRegistro", "tipoNota_id","usuarioRegistro_id" ,"ripsEnvio_id","sedesClinica_id" ,"numFactura" ,"estadoReg") select substring(sed.nit,1,9) ,  nc.id, now(), tipnot.id,' + "'" + str(username_id) + "'" + ' , e.id, sed.id , ' + "'" + str(elementoOtro) + "','" + str('A') + "'" + ' from sitios_sedesclinica sed, cartera_notascredito nc, cartera_notascreditodetalle ncDet, rips_ripsEnvios e  , rips_ripsdetalle det ,rips_ripstiposnotas tipnot where e.id = ' + "'" + str(envioRipsId) + "'" + ' and e."sedesClinica_id" = sed.id and nc."ripsEnvio_id" = e.id and det."ripsEnvios_id" = e.id and e."ripsTiposNotas_id" = tipnot.id and tipnot.nombre=' + "'" + str('Nota Credito') + "'" + ' AND ncDet.factura_id = ' + "'" + str(elementoOtro) + "'"  + ' and nc.id = ncDet."notaCredito_id" and det."notaCredito_id" = nc.id and det."numeroFactura_id" = ncDet.factura_id  AND ncDet."notaCredito_id"= ' + "'" + str(elemento) + "'" + ' RETURNING id ;'
                ...
                print ("Voy a usuarios transaccionId = " , transaccionId )
                ...
                print ("detalle de Usuarios = ", detalle)
                curx.execute(detalle)
            ...
            # Busca cantidad de Medicamentos +  Suministros para proratear en ABONOS elemento= factura , elementoOtro=glosa
                ...
                try:
                    with transaction.atomic():
                        ...
                        tipoModeradora = FormasPagos.objects.get(nombre='CUOTA MODERADORA')
                        tipoCopago = FormasPagos.objects.get(nombre='COPAGO')
                except Exception as e:
                    # Aquí ya se hizo rollback automáticamente
                    print("Se hizo rollback por:", e)

                detalle = 'select formaPago.id id , formaPago.nombre nombre , sum(fac."valorAplicado") valorAplicado from cartera_pagosfacturas fac INNER JOIN cartera_pagos pagos ON (pagos.id=fac.pago_id) INNER JOIN cartera_formaspagos formaPago ON (formaPago.id = pagos."formaPago_id") WHERE fac ."facturaAplicada_id" = ' + "'" + str(elementoOtro) +  "'" + ' and fac.anulado = ' + "'" + str('N') + "'  GROUP BY formaPago.id, formaPago.nombre ORDER BY formaPago.id"

                curx.execute(detalle)
                abonos=[]

                for id, nombre, valorAplicado  in curx.fetchall():
                   abonos.append({'id': id, 'nombre':nombre, "valorAplicado":valorAplicado})
                ...
                if (tipoRips == 'Factura'):
                    ...
                    #detalle1 = ' INSERT INTO rips_ripsprocedimientos ("codPrestador", "fechaInicioAtencion", "idMIPRES", "numAutorizacion","numDocumentoIdentificacion", "vrServicio",	"valorPagoModerador", "numFEVPagoModerador", consecutivo, "fechaRegistro", "codComplicacion_id", "codDiagnosticoPrincipal_id","codDiagnosticoRelacionado_id", "codProcedimiento_id", "codServicio_id", "conceptoRecaudo_id", "finalidadTecnologiaSalud_id",	"grupoServicios_id", "modalidadGrupoServicioTecSal_id", "tipoDocumentoIdentificacion_id","usuarioRegistro_id", "viaIngresoServicioSalud_id", "ripsDetalle_id", "itemFactura", "ripsTipos_id", "tipoPagoModerador_id", "ripsTransaccion_id", "estadoReg", ingreso_id )  SELECT sed."codigoHabilitacion", facdet."fecha", facdet.mipres, (select autDet."numeroAutorizacion" autDet  from  autorizaciones_autorizacionesdetalle autdet  where autdet.id = facdet."autorizacionDetalle_id") , usu.documento, facdet."valorTotal", ' + str(proRata) + ', fac.id, row_number()  OVER(ORDER BY facdet.id) + ' + str(traigoConsecutivo) + ' AS consecutivo, now(), (select max(diag4.id) from clinico_diagnosticos diag4 where  diag4.id = i."dxComplicacion_id"), (select  max(diag1.id) from clinico_historia his ,clinico_historialdiagnosticos histdiag1, clinico_diagnosticos diag1 where his."tipoDoc_id" = i."tipoDoc_id" AND his.documento_id = i.documento_id and his."consecAdmision" = i.consec AND histdiag1.historia_id = his.id and histdiag1."tiposDiagnostico_id" = ' + "'" + str('1') + "'" + '),(select  max(diag1.id) from clinico_historia his ,clinico_historialdiagnosticos histdiag1, clinico_diagnosticos diag1 where his."tipoDoc_id" = i."tipoDoc_id" AND his.documento_id = i.documento_id and his."consecAdmision" = i.consec AND histdiag1.historia_id = his.id and histdiag1."tiposDiagnostico_id" = ' + "'" + str('2')  + "'" + '), exa.id, serv.id,' + str('5')  + '  , final.id, gru.id, mod.id, tipdocrips.id, ' + "'" + str(username_id) + "'" + ', ingreso.id, detrips.id, facdet."consecutivoFactura", ' + "'" + str('4') + "'" + ', (select max(ripsmoderadora.id) from cartera_pagos pagos, cartera_formaspagos formapago, rips_ripstipospagomoderador ripsmoderadora where i."tipoDoc_id" = pagos."tipoDoc_id" and i.documento_id = pagos.documento_id and i.consec = pagos.consec and pagos."formaPago_id" = formapago.id and ripsmoderadora."codigoAplicativo" = cast(formapago.id as text)), ' + "'" + str(transaccionId) + "','A','" + str(ingresoId.id) + "'"  + ' FROM sitios_sedesclinica sed inner join facturacion_facturacion fac ON (fac."sedesClinica_id" = sed.id) inner join facturacion_facturaciondetalle facdet ON (facdet.facturacion_id = fac.id and facdet."examen_id" is not null and (facdet.anulado = ' + "'" + str('N') + "'" + ' or facdet.anulado = ' + "'" + str('R') + "')" + ' and "tipoRegistro" = ' + "'" + str('SISTEMA') + "' and facdet.cirugia_id is null)" + ' inner join clinico_examenes exa ON (exa.id = facdet."examen_id") inner join admisiones_ingresos i on (i."tipoDoc_id" = fac."tipoDoc_id" and i.documento_id = fac.documento_id and i.consec = fac."consecAdmision") left join rips_ripsviasingresosalud ingreso ON (ingreso.id = i."ripsViaIngresoServicioSalud_id") inner join rips_ripsenvios e ON (e."sedesClinica_id" = sed.id) inner join rips_ripsdetalle detrips ON (detrips."ripsEnvios_id" = e.id and detrips."numeroFactura_id" = fac.id) left join rips_ripsmodalidadatencion mod ON (mod.id = i."ripsmodalidadGrupoServicioTecSal_id") left join  rips_ripsgruposervicios gru ON (gru.id = i."ripsGrupoServicios_id")    left join rips_ripsServicios serv ON (serv.id = i."ripsGrupoServicios_id")  left join  rips_ripsfinalidadconsulta final on (final.id = i."ripsFinalidadConsulta_id") inner join usuarios_tiposdocumento tipdoc ON (tipdoc.id = fac."tipoDoc_id" ) left join  rips_ripstiposdocumento tipdocrips on (tipdocrips.id = tipdoc."tipoDocRips_id" ) inner join usuarios_usuarios usu ON (usu."tipoDoc_id" = fac."tipoDoc_id" and usu.id = fac.documento_id )  where sed.id = ' + "'" + str(sede) + "'" + ' and e.id = ' + "'" + str(envioRipsId) + "'" + ' and fac.id = ' + "'" + str(elemento) + "' and facdet.examen_id is not null"


                    detalle1 = ' INSERT INTO rips_ripsprocedimientos ("codPrestador", "fechaInicioAtencion", "idMIPRES", "numAutorizacion","numDocumentoIdentificacion", "vrServicio",	"valorPagoModerador", "numFEVPagoModerador", consecutivo, "fechaRegistro", "codComplicacion_id", "codDiagnosticoPrincipal_id","codDiagnosticoRelacionado_id", "codProcedimiento_id", "codServicio_id", "conceptoRecaudo_id", "finalidadTecnologiaSalud_id",	"grupoServicios_id", "modalidadGrupoServicioTecSal_id", "tipoDocumentoIdentificacion_id","usuarioRegistro_id", "viaIngresoServicioSalud_id", "ripsDetalle_id", "itemFactura", "ripsTipos_id", "tipoPagoModerador_id", "ripsTransaccion_id", "estadoReg", ingreso_id )  SELECT sed."codigoHabilitacion", facdet."fecha", facdet.mipres, (select autDet."numeroAutorizacion" autDet  from  autorizaciones_autorizacionesdetalle autdet  where autdet.id = facdet."autorizacionDetalle_id") , usu.documento, facdet."valorTotal", null, fac.id, row_number()  OVER(ORDER BY facdet.id) + ' + str(traigoConsecutivo) + ' AS consecutivo, now(), (select max(diag4.id) from clinico_diagnosticos diag4 where  diag4.id = i."dxComplicacion_id"), (select  max(diag1.id) from clinico_historia his ,clinico_historialdiagnosticos histdiag1, clinico_diagnosticos diag1 where his."tipoDoc_id" = i."tipoDoc_id" AND his.documento_id = i.documento_id and his."consecAdmision" = i.consec AND histdiag1.historia_id = his.id and histdiag1."tiposDiagnostico_id" = ' + "'" + str('1') + "'" + '),(select  max(diag1.id) from clinico_historia his ,clinico_historialdiagnosticos histdiag1, clinico_diagnosticos diag1 where his."tipoDoc_id" = i."tipoDoc_id" AND his.documento_id = i.documento_id and his."consecAdmision" = i.consec AND histdiag1.historia_id = his.id and histdiag1."tiposDiagnostico_id" = ' + "'" + str('2')  + "'" + '), exa.id, serv.id,' + str('5')  + '  , final.id, gru.id, mod.id, tipdocrips.id, ' + "'" + str(username_id) + "'" + ', ingreso.id, detrips.id, facdet."consecutivoFactura", ' + "'" + str('4') + "',null," + "'" + str(transaccionId) + "','A','" + str(ingresoId.id) + "'"  + ' FROM sitios_sedesclinica sed inner join facturacion_facturacion fac ON (fac."sedesClinica_id" = sed.id) inner join facturacion_facturaciondetalle facdet ON (facdet.facturacion_id = fac.id and facdet."examen_id" is not null and (facdet.anulado = ' + "'" + str('N') + "'" + ' or facdet.anulado = ' + "'" + str('R') + "')" + ' and "tipoRegistro" = ' + "'" + str('SISTEMA') + "' and facdet.cirugia_id is null)" + ' inner join clinico_examenes exa ON (exa.id = facdet."examen_id") inner join admisiones_ingresos i on (i."tipoDoc_id" = fac."tipoDoc_id" and i.documento_id = fac.documento_id and i.consec = fac."consecAdmision") left join rips_ripsviasingresosalud ingreso ON (ingreso.id = i."ripsViaIngresoServicioSalud_id") inner join rips_ripsenvios e ON (e."sedesClinica_id" = sed.id) inner join rips_ripsdetalle detrips ON (detrips."ripsEnvios_id" = e.id and detrips."numeroFactura_id" = fac.id) left join rips_ripsmodalidadatencion mod ON (mod.id = i."ripsmodalidadGrupoServicioTecSal_id") left join  rips_ripsgruposervicios gru ON (gru.id = i."ripsGrupoServicios_id")    left join rips_ripsServicios serv ON (serv.id = i."ripsGrupoServicios_id")  left join  rips_ripsfinalidadconsulta final on (final.id = i."ripsFinalidadConsulta_id") inner join usuarios_tiposdocumento tipdoc ON (tipdoc.id = fac."tipoDoc_id" ) left join  rips_ripstiposdocumento tipdocrips on (tipdocrips.id = tipdoc."tipoDocRips_id" ) inner join usuarios_usuarios usu ON (usu."tipoDoc_id" = fac."tipoDoc_id" and usu.id = fac.documento_id )  where sed.id = ' + "'" + str(sede) + "'" + ' and e.id = ' + "'" + str(envioRipsId) + "'" + ' and fac.id = ' + "'" + str(elemento) + "' and facdet.examen_id is not null"



                    print("detalleProc2 = ", detalle1)
                ...
                if (tipoRips == 'Glosa'):
                    print ("Entro glosa proced")

                    detalle = 'INSERT INTO rips_ripsprocedimientos("glosa_id","codPrestador", "fechaInicioAtencion", "idMIPRES", "numAutorizacion","numDocumentoIdentificacion", "vrServicio","valorPagoModerador", "numFEVPagoModerador", consecutivo, "fechaRegistro", "codComplicacion_id", "codDiagnosticoPrincipal_id","codDiagnosticoRelacionado_id", "codProcedimiento_id", "codServicio_id", "conceptoRecaudo_id", "finalidadTecnologiaSalud_id",	"grupoServicios_id", "modalidadGrupoServicioTecSal_id","tipoDocumentoIdentificacion_id","usuarioRegistro_id", "viaIngresoServicioSalud_id", "ripsDetalle_id", "itemFactura", "ripsTipos_id", "tipoPagoModerador_id", "ripsTransaccion_id", "estadoReg", ingreso_id , "notasCreditoGlosa") SELECT glosas.id, "codPrestador", "fechaInicioAtencion", "idMIPRES", "numAutorizacion","numDocumentoIdentificacion", "vrServicio","valorPagoModerador", "numFEVPagoModerador", row_number() OVER(ORDER BY proc.id) AS consecutivo , ' + "'" + str(fechaRegistro) + "'" + ' , "codComplicacion_id", "codDiagnosticoPrincipal_id","codDiagnosticoRelacionado_id", "codProcedimiento_id", "codServicio_id", "conceptoRecaudo_id", "finalidadTecnologiaSalud_id",	"grupoServicios_id", "modalidadGrupoServicioTecSal_id","tipoDocumentoIdentificacion_id", ' + "'" + str(username_id) + "'" + ' , "viaIngresoServicioSalud_id",' + "'" + str(elementox['id']) + "'" + ', proc."itemFactura", proc."ripsTipos_id", 	proc."tipoPagoModerador_id",' + "'" +  str(transaccionId) + "'," + "'A'," + "'" + str(ingresoId.id) + "'," +  ' gloDetRips."valorAceptado" FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsProcedimientos proc on (' + str('1=1') + ')  inner join cartera_glosasdetallerips gloDetRips on (gloDetRips."ripsProcedimientos_id" = proc.id) inner join cartera_glosasDetalle gloDet on (gloDet.id =  gloDetRips."glosasDetalle_id") inner join cartera_glosas glosas on (glosas.id = gloDet."glosa_id") inner join rips_ripsdetalle det on (det."ripsEnvios_id" = env.id and  det."numeroFactura_id" =  gloDet.factura_id and  det."glosa_id" = ' + "'" + str(elemento) + "'" + ' AND det."glosa_id" = gloDetRips."glosasDetalle_id" AND rips_ripstransaccion."numNota" =  cast(det.glosa_id as text)) where proc."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'"
                ...
                if (tipoRips == 'Nota Credito'):
                    ...
                    detalle = 'INSERT INTO rips_ripsprocedimientos("notaCredito_id","codPrestador", "fechaInicioAtencion", "idMIPRES", "numAutorizacion","numDocumentoIdentificacion", "vrServicio","valorPagoModerador", "numFEVPagoModerador", consecutivo, "fechaRegistro", "codComplicacion_id", "codDiagnosticoPrincipal_id","codDiagnosticoRelacionado_id", "codProcedimiento_id", "codServicio_id", "conceptoRecaudo_id", "finalidadTecnologiaSalud_id",	"grupoServicios_id", "modalidadGrupoServicioTecSal_id","tipoDocumentoIdentificacion_id","usuarioRegistro_id", "viaIngresoServicioSalud_id", "ripsDetalle_id", "itemFactura", "ripsTipos_id", "tipoPagoModerador_id", "ripsTransaccion_id", "estadoReg", ingreso_id , "notasCreditoOtras") SELECT nc.id, "codPrestador", "fechaInicioAtencion", "idMIPRES", "numAutorizacion","numDocumentoIdentificacion", "vrServicio","valorPagoModerador", "numFEVPagoModerador", row_number() OVER(ORDER BY proc.id) AS consecutivo , ' + "'" + str(fechaRegistro) + "'" + ' , "codComplicacion_id", "codDiagnosticoPrincipal_id","codDiagnosticoRelacionado_id", "codProcedimiento_id", "codServicio_id", "conceptoRecaudo_id", "finalidadTecnologiaSalud_id",	"grupoServicios_id", "modalidadGrupoServicioTecSal_id","tipoDocumentoIdentificacion_id", ' + "'" + str(username_id) + "'" + ' , "viaIngresoServicioSalud_id",' + "'" + str(elementox['id']) + "'" + ', proc."itemFactura", proc."ripsTipos_id", 	proc."tipoPagoModerador_id",' + "'" +  str(transaccionId) + "'," + "'A'," + "'" + str(ingresoId.id) + "'," +  ' ncDetRips."valorNota" FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsProcedimientos proc on (' + str('1=1') + ') inner join cartera_notascreditodetallerips ncDetRips on (ncDetRips."ripsProcedimientos_id" = proc.id)  inner join cartera_notascreditoDetalle ncDet on (ncDet.id =  ncDetRips."notaCreditoDetalle_id" and ncDet."notaCredito_id" = cast(rips_ripstransaccion."numNota" as integer) ) inner join cartera_notascredito nc on (nc.id = ncDet."notaCredito_id") inner join rips_ripsdetalle det on (det."ripsEnvios_id" = env.id and det."numeroFactura_id" =  ncDet.factura_id and  det."notaCredito_id" = nc.id AND rips_ripstransaccion."numNota" =  cast(det."notaCredito_id" as text)) where proc."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'" + ' and rips_ripstransaccion.id=' + str(transaccionId)
                    print("detalle PROCEDIMIENTOS= ", detalle)
                    curx.execute(detalle)
                ...
                if (tipoRips == 'Factura'):
                    ...
                    detalle1 = 'INSERT INTO rips_ripsmedicamentos ("codPrestador", "numAutorizacion", "idMIPRES", "fechaDispensAdmon", "nomTecnologiaSalud", "concentracionMedicamento", "cantidadMedicamento", 	"diasTratamiento",	"numDocumentoIdentificacion", "vrUnitMedicamento", "vrServicio","valorPagoModerador", "numFEVPagoModerador",consecutivo, "fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id","codTecnologiaSalud_id", "conceptoRecaudo_id", "formaFarmaceutica_id", "tipoDocumentoIdentificacion_id", "tipoMedicamento_id", "unidadMedida_id","unidadMinDispensa_id", "usuarioRegistro_id", "ripsDetalle_id", "itemFactura","ripsTipos_id", "ripsTransaccion_id", "estadoReg", ingreso_id , "tipoPagoModerador_id" ) SELECT sed."codigoHabilitacion", null, facdet.mipres, facdet.fecha,null,null ,null ,null ,planta.documento, facdet."valorUnitario",	facdet."valorTotal", 0,  fac.id, row_number() OVER(ORDER BY facdet.id)  + ' + str(traigoConsecutivo) + ' , now(),diag1.id , diag2.id, ripscums.id, null, ripsfarma.id, ripstipdoc.id, tipmed.id, ripsumm.id, ripsupr.id, ' + "'" + str(username_id) + "'" + ' , det.id, facdet."consecutivoFactura", ' + "'" + str('8') + "'" + ' , rips_ripstransaccion.id , ' + "'" + str('A') + "','" + str(ingresoId.id) + "',null"  + ' from rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id") inner join rips_ripsdetalle det on (det."ripsEnvios_id" = env.id and det."numeroFactura_id" = cast(rips_ripstransaccion."numFactura" as numeric)) inner join facturacion_facturacion fac on (fac.id = det."numeroFactura_id") inner join facturacion_facturaciondetalle facdet on (facdet."facturacion_id" = fac.id and facdet."cums_id" is not null and (facdet.anulado = ' + "'" + str('N') + "'" + ' or facdet.anulado = ' + "'" + str('R') + "')" + ' AND facDet."tipoRegistro" = ' + "'" + str('MANUAL') + "')" + ' inner join facturacion_suministros sum on (sum.id = facdet.cums_id) inner join facturacion_tipossuministro tiposSum ON (tiposSum.id=sum."tipoSuministro_id" and tiposSum.nombre=' + "'" + str(tipoMedicamento.nombre) + "')" + '   left join rips_ripstipomedicamento tipmed on (tipmed.id = sum."ripsTipoMedicamento_id" ) inner join rips_ripscums ripscums  on (ripscums.cum = sum."cums") left join rips_ripsumm ripsumm on (ripsumm.id = sum."ripsUnidadMedida_id") left join rips_RipsFormaFarmaceutica ripsfarma on (ripsfarma.id = sum."ripsFormaFarmaceutica_id") left join rips_ripsunidadupr ripsupr on (ripsupr.id = sum."ripsUnidadUpr_id") inner join admisiones_ingresos adm on (adm."tipoDoc_id" = fac."tipoDoc_id" and adm.documento_id=fac.documento_id and adm.consec=fac."consecAdmision") left join 	clinico_medicos medico on (medico.id =adm."medicoActual_id") inner join planta_planta planta on (planta.id = medico.planta_id) left join usuarios_tiposdocumento usutipdoc on (usutipdoc.id = planta."tipoDoc_id") left join rips_ripstiposdocumento ripstipdoc on (ripstipdoc.id = usutipdoc."tipoDocRips_id")   left join clinico_diagnosticos diag1 on (diag1.id=adm."dxActual_id") left join clinico_diagnosticos diag2 on (diag2.id=adm."dxSalida_id") where env.id =  ' + "'" + str(envioRipsId) + "'" + ' and rips_ripstransaccion."ripsEnvio_id" = env.id  and cast(rips_ripstransaccion."numFactura" as numeric) = fac.id  and fac.id = ' + "'" + str(elemento) + "' AND facdet.cums_id is not null"
                    curx.execute(detalle1)
                    print("detalle1 =", detalle1)
                ...
                if (tipoRips == 'Glosa'):
                    ...
                    detalle = 'INSERT INTO rips_ripsmedicamentos ("glosa_id","codPrestador", "numAutorizacion", "idMIPRES", "fechaDispensAdmon", "nomTecnologiaSalud", "concentracionMedicamento", "cantidadMedicamento", "diasTratamiento",	"numDocumentoIdentificacion", "vrUnitMedicamento", "vrServicio", "valorPagoModerador", "numFEVPagoModerador",consecutivo, "fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id", "formaFarmaceutica_id", "tipoDocumentoIdentificacion_id","tipoMedicamento_id", "unidadMedida_id", "unidadMinDispensa_id", "usuarioRegistro_id", "ripsDetalle_id", "itemFactura","ripsTipos_id", "ripsTransaccion_id", ingreso_id, "estadoReg" , "notasCreditoGlosa") SELECT glosas.id, "codPrestador", "numAutorizacion", "idMIPRES", "fechaDispensAdmon", "nomTecnologiaSalud", "concentracionMedicamento", "cantidadMedicamento", "diasTratamiento","numDocumentoIdentificacion", "vrUnitMedicamento", "vrServicio", "valorPagoModerador", "numFEVPagoModerador",row_number() OVER(ORDER BY med.id) AS consecutivo , med."fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id", "formaFarmaceutica_id", "tipoDocumentoIdentificacion_id","tipoMedicamento_id", "unidadMedida_id", "unidadMinDispensa_id", med."usuarioRegistro_id", ' + "'" + str(elementox['id']) + "'" + ' , med."itemFactura",med."ripsTipos_id", ' + "'" + str(transaccionId) + "','"  + str(ingresoId.id) + "','A'"  + ', gloDetRips."valorAceptado"  FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsMedicamentos med on (' + str('1=1') + ')  inner join cartera_glosasdetallerips gloDetRips  ON (gloDetRips."ripsMedicamentos_id" = med.id)  inner join cartera_glosasdetalle  gloDet on (gloDet.id = gloDetRips."glosasDetalle_id") inner join cartera_glosas glosas on (glosas.id = gloDet."glosa_id" and gloDet.factura_id = cast(med."numFEVPagoModerador"  as integer)) inner join rips_ripsdetalle det on (det."ripsEnvios_id" = env.id and det."numeroFactura_id" =  cast(med."numFEVPagoModerador"  as integer) and  det."glosa_id" = glosas.id and det."glosa_id"=' + "'" + str(elemento) + "'" + ' AND det."glosa_id" = gloDetRips."glosasDetalle_id" AND rips_ripstransaccion."numNota" =  cast(det.glosa_id as text) )  where med."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'"
                ...
                if (tipoRips == 'Nota Credito'):
                    ...
                    detalle = 'INSERT INTO rips_ripsmedicamentos ("notaCredito_id","codPrestador", "numAutorizacion", "idMIPRES", "fechaDispensAdmon", "nomTecnologiaSalud", "concentracionMedicamento", "cantidadMedicamento", "diasTratamiento",	"numDocumentoIdentificacion", "vrUnitMedicamento", "vrServicio", "valorPagoModerador", "numFEVPagoModerador",consecutivo, "fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id", "formaFarmaceutica_id", "tipoDocumentoIdentificacion_id","tipoMedicamento_id", "unidadMedida_id", "unidadMinDispensa_id", "usuarioRegistro_id", "ripsDetalle_id", "itemFactura","ripsTipos_id", "ripsTransaccion_id", ingreso_id, "estadoReg" , "notasCreditoOtras") SELECT nc.id, "codPrestador", "numAutorizacion", "idMIPRES", "fechaDispensAdmon", "nomTecnologiaSalud", "concentracionMedicamento", "cantidadMedicamento", "diasTratamiento","numDocumentoIdentificacion", "vrUnitMedicamento", "vrServicio", "valorPagoModerador", "numFEVPagoModerador",row_number() OVER(ORDER BY med.id) AS consecutivo , med."fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id", "formaFarmaceutica_id", "tipoDocumentoIdentificacion_id","tipoMedicamento_id", "unidadMedida_id", "unidadMinDispensa_id", med."usuarioRegistro_id", ' + "'" + str(elementox['id']) + "'" + ' , med."itemFactura",med."ripsTipos_id", ' + "'" + str(transaccionId) + "','"  + str(ingresoId.id) + "','A'"  + ', ncDetRips."valorNota"  FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsMedicamentos med on (' + str('1=1') + ')  inner join cartera_notascreditodetallerips ncDetRips  ON (ncDetRips."ripsMedicamentos_id" = med.id)  inner join cartera_notascreditodetalle  ncDet on (ncDet.id = ncDetRips."notaCreditoDetalle_id" ) inner join cartera_notascredito nc on (nc.id = ncDet."notaCredito_id" and ncDet.factura_id = cast(med."numFEVPagoModerador"  as integer)) inner join rips_ripsdetalle det on (det."ripsEnvios_id" = env.id and  det."numeroFactura_id" =  cast(med."numFEVPagoModerador"  as integer) and  det."notaCredito_id" =nc.id and det."notaCredito_id"=' + "'" + str(elemento) + "'" + ' AND rips_ripstransaccion."numNota" =  cast(det."notaCredito_id" as text))  where med."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'"
                ...
                if (tipoRips == 'Factura'):
                    ...
                    print("CREO AQUI VAMOS_0")
                    print("traigoConsecutivoId", traigoConsecutivo)

                    print("CREO AQUI VAMOS_1")

                    # Cirugia INSUMOS manual
                    detalle = 'INSERT INTO rips_ripsotrosservicios ( "codPrestador", "numAutorizacion", "idMIPRES", "fechaSuministroTecnologia","tipoOS_id", "codTecnologiaSalud_id","nomTecnologiaSalud", "cantidadOS",     "tipoDocumentoIdentificacion_id", "numDocumentoIdentificacion", "vrUnitOS", "vrServicio",       "tipoPagoModerador_id", "valorPagoModerador","numFEVPagoModerador", consecutivo, "fechaRegistro",   "usuarioRegistro_id",       "ripsDetalle_id", "itemFactura", "ripsTipos_id", "ripsTransaccion_id", "estadoReg", ingreso_id) SELECT sed."codigoHabilitacion", facdet."autorizacionDetalle_id", autdet.mipres, facdet."fecha",   ripsOtros.id,  exa.id,  substring(exa.nombre,1,60), facdet.cantidad,        tipdocrips.id, usu.documento,facdet."valorUnitario",    facdet."valorTotal", null,null,  fac.id, row_number()  OVER(ORDER BY facdet.id) + ' + str(traigoConsecutivo) + '  AS consecutivo, now(), ' + "'" + str(username_id) + "'" + ',detrips.id,facdet."consecutivoFactura",' + "'" + str('9') + "'" + ',' + str(transaccionId) + ",'" + str('A') + "'" + ',i.id  FROM sitios_sedesclinica sed inner join facturacion_facturacion fac ON (fac."sedesClinica_id" = sed.id) inner join facturacion_facturaciondetalle facdet ON (facdet.facturacion_id = fac.id and facdet."cums_id" is not null and (facdet.anulado = ' + "'" + str('N') + "'" + ' or facdet.anulado = ' + "'" + str('R') + "'" + ') and "tipoRegistro" = ' + "'" + str('MANUAL') + "'" + ' AND facDet.cirugia_id is not null) inner join facturacion_suministros exa ON (exa.id = facdet."cums_id") inner join admisiones_ingresos i on (i."tipoDoc_id" = fac."tipoDoc_id" and i.documento_id = fac.documento_id and i.consec = fac."consecAdmision") inner join rips_ripsenvios e ON (e."sedesClinica_id" = sed.id)  inner join rips_ripsdetalle detrips ON (detrips."ripsEnvios_id" = e.id and detrips."numeroFactura_id" = fac.id) inner join usuarios_tiposdocumento tipdoc ON (tipdoc.id = fac."tipoDoc_id" ) left join  rips_ripstiposdocumento tipdocrips on (tipdocrips.id = tipdoc."tipoDocRips_id" ) 	inner join usuarios_usuarios usu ON (usu."tipoDoc_id" = fac."tipoDoc_id" and usu.id = fac.documento_id ) left join clinico_historia his ON (his."tipoDoc_id" = i."tipoDoc_id" and his.documento_id = i.documento_id and his."consecAdmision" = i.consec ) inner join clinico_historialcirugias hisCiru ON (hisCiru.historia_id = his.id and hisCiru.cirugia_id = facDet.cirugia_id) left join autorizaciones_autorizaciones  aut on (aut.ingreso_id = i.id) left join autorizaciones_estadosautorizacion autEst on (autEst.nombre=' + "'" + str('AUTORIZADO') + "'" + ')  left join autorizaciones_autorizacionesdetalle autdet on (autdet.autorizaciones_id = aut.id and autdet.examenes_id = facdet.examen_id AND autdet."estadoAutorizacion_id" = autEst.id and autdet.id = facdet."autorizacionDetalle_id") inner join tarifarios_tiposhonorarios tipHonor on ( tipHonor.id = facDet."tipoHonorario_id" and tipHonor.nombre =' + "'" + str('MATERIALES DE SUTURA Y CURACIO') + "')" + ' inner join rips_ripstipootrosservicios ripsOtros on (ripsOtros.id=tipHonor."ripsTipoOtrosServicios_id") where sed.id = ' + "'" + str(sede) + "'" + ' and e.id = ' + "'" + str(envioRipsId) + "'" + ' and fac.id = ' + "'" + str(elemento) + "'"
                    ...
                    print("CREO AQUI VAMOS_3")
                    print("totalOtros =" , totalOtros1)
                    print("CREO AQUI VAMOS_2")
                    print("totaltotalOtros1 =", totalOtros1[0]['totalOtros'])
                    print("CREO AQUI VAMOS_4")
                    totalOtros = str(totalOtros1[0]['totalOtros'])
                    totalOtros = totalOtros.replace("(", ' ')
                    totalOtros = totalOtros.replace(")", ' ')
                    totalOtros = totalOtros.replace(",", ' ')
                    ...
                    totalOtros = totalOtros.replace(",", ' ')
                    traigoConsecutivo = totalOtros
                    print("CREO AQUI VAMOS_5")
                    print("traigoConsecutivoId", traigoConsecutivo)
                    print("CREO AQUI VAMOS_6")

                   

		    # AQUI HONORARIOS MANUAL
                    ...
                    totalOtros = totalOtros.replace(",", ' ')
                    traigoConsecutivo = totalOtros
                    print("CREO AQUI VAMOS_7")
                    print("traigoConsecutivoId", traigoConsecutivo)
                    print("CREO AQUI VAMOS_8")

                    conceptoEstancias = Conceptos.objects.get(nombre='ESTANCIAS')
		    ...
		    #Estancias con MANUAL
                ...
                if (tipoRips == 'Glosa'):
                    ...
                    detalle = 'INSERT INTO rips_ripsotrosservicios ("glosa_id","codPrestador", "numAutorizacion", "idMIPRES", "fechaSuministroTecnologia", "nomTecnologiaSalud", "cantidadOS", "numDocumentoIdentificacion", "vrUnitOS", "vrServicio", "valorPagoModerador", "numFEVPagoModerador", consecutivo, "fechaRegistro", "codTecnologiaSaludCups_id", "conceptoRecaudo_id", "tipoDocumentoIdentificacion_id", "tipoOS_id", "usuarioRegistro_id", "ripsDetalle_id", "itemFactura", "ripsTipos_id", "ripsTransaccion_id", "notasCreditoGlosa","estadoReg", ingreso_id, "tipoPagoModerador_id") SELECT glosas.id,"codPrestador", "numAutorizacion", "idMIPRES", "fechaSuministroTecnologia", "nomTecnologiaSalud", "cantidadOS", "numDocumentoIdentificacion", "vrUnitOS", "vrServicio", "valorPagoModerador", "numFEVPagoModerador", row_number() OVER(ORDER BY ripsOtros.id) AS consecutivo , ripsOtros."fechaRegistro", "codTecnologiaSaludCups_id", "conceptoRecaudo_id", "tipoDocumentoIdentificacion_id", "tipoOS_id",ripsOtros."usuarioRegistro_id",' + "'" + str(
                        elementox['id']) + "'" + ', ripsOtros."itemFactura", ripsOtros."ripsTipos_id", ' + "'" + str(
                        transaccionId) + "'," + 'gloDetRips."valorNotasCredito" ' + ",'A','" + str(
                        ingresoId.id) + "'," + '"tipoPagoModerador_id" FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsOtrosServicios ripsOtros on (' + str('1=1') + ')   inner join cartera_glosasdetallerips gloDetRips ON (gloDetRips."ripsOtrosServicios_id" = ripsOtros.id) inner join cartera_glosasdetalle gloDet on (gloDet.id = gloDetRips."glosasDetalle_id") inner join cartera_glosas glosas on (glosas.id = gloDet."glosa_id" and gloDet.factura_id = cast(ripsOtros."numFEVPagoModerador"  as integer)) inner join rips_ripsdetalle det on (det."ripsEnvios_id" = env.id and  det."numeroFactura_id" =  cast(ripsOtros."numFEVPagoModerador" as integer) and det."glosa_id"=glosas.id and det."glosa_id" =' + "'" + str(
                        elemento) + "'" + ' AND det."glosa_id" = gloDetRips."glosasDetalle_id" AND rips_ripstransaccion."numNota" =  cast(det.glosa_id as text)) where ripsOtros."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'"
                ...
                if (tipoRips == 'Nota Credito'):
                    ...
                    detalle = 'INSERT INTO rips_ripsotrosservicios ("notaCredito_id","codPrestador", "numAutorizacion", "idMIPRES", "fechaSuministroTecnologia", "nomTecnologiaSalud", "cantidadOS", "numDocumentoIdentificacion", "vrUnitOS", "vrServicio", "valorPagoModerador", "numFEVPagoModerador", consecutivo, "fechaRegistro", "codTecnologiaSaludCups_id", "conceptoRecaudo_id", "tipoDocumentoIdentificacion_id", "tipoOS_id", "usuarioRegistro_id", "ripsDetalle_id", "itemFactura", "ripsTipos_id", "ripsTransaccion_id", "notasCreditoOtras","estadoReg", ingreso_id, "tipoPagoModerador_id") SELECT nc.id,"codPrestador", "numAutorizacion", "idMIPRES", "fechaSuministroTecnologia", "nomTecnologiaSalud", "cantidadOS", "numDocumentoIdentificacion", "vrUnitOS", "vrServicio", "valorPagoModerador", "numFEVPagoModerador", row_number() OVER(ORDER BY ripsOtros.id) AS consecutivo , ripsOtros."fechaRegistro", "codTecnologiaSaludCups_id", "conceptoRecaudo_id", "tipoDocumentoIdentificacion_id", "tipoOS_id",ripsOtros."usuarioRegistro_id",' + "'" + str(
                        ...
                        ingresoId.id) + "'," + '"tipoPagoModerador_id" FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsOtrosServicios ripsOtros on (' + str('1=1') + ')  inner join cartera_notascreditodetallerips ncDetRips ON (ncDetRips."ripsOtrosServicios_id" = ripsOtros.id) inner join cartera_notascreditodetalle ncDet on (ncDet.id = ncDetRips."notaCreditoDetalle_id") inner join cartera_notascredito nc on (nc.id = ncDet."notaCredito_id" and ncDet.factura_id = cast(ripsOtros."numFEVPagoModerador"  as integer)) inner join rips_ripsdetalle det on (det."ripsEnvios_id" = env.id and det."numeroFactura_id" =  cast(ripsOtros."numFEVPagoModerador" as integer) and det."notaCredito_id"=nc.id and det."notaCredito_id" =' + "'" + str(
                        elemento) + "'" + ' AND rips_ripstransaccion."numNota" =  cast(det."notaCredito_id" as text)) where ripsOtros."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'"
                    print("detalle = ", detalle)
                    curx.execute(detalle)
                ...
                if (tipoRips == 'Glosa'):
                    ...
                    detalle = 'INSERT INTO rips_ripsconsultas ("glosa_id","codPrestador", "numAutorizacion", 	"numDocumentoIdentificacion",  "vrServicio", "valorPagoModerador", "numFEVPagoModerador",consecutivo, "fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id",  "tipoDocumentoIdentificacion_id", "usuarioRegistro_id", "ripsDetalle_id", "itemFactura","ripsTipos_id", "ripsTransaccion_id", ingreso_id, "estadoReg" , "notasCreditoGlosa") SELECT glosas.id, "codPrestador", "numAutorizacion","numDocumentoIdentificacion", "vrServicio", "valorPagoModerador", "numFEVPagoModerador",row_number() OVER(ORDER BY med.id) AS consecutivo , med."fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id",  "tipoDocumentoIdentificacion_id", med."usuarioRegistro_id", ' + "'" + str(elementox['id']) + "'" + ' , med."itemFactura",med."ripsTipos_id", ' + "'" + str(transaccionId) + "','"  + str(ingresoId) + "','A'"  + ', gloDetRips."valorAceptado"  FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsconsultas cons on (' + str('1=1') + ')  inner join cartera_glosasdetallerips gloDetRips  ON (gloDetRips."ripsConsultas_id" = cons.id)  inner join cartera_glosasdetalle  gloDet on (gloDet.id = gloDetRips."glosasDetalle_id") inner join cartera_glosas glosas on (glosas.id = gloDet."glosa_id" and gloDet.factura_id = cast(cons."numFEVPagoModerador"  as integer)) inner join rips_ripsdetalle det on (det."numeroFactura_id" =  cast(cons."numFEVPagoModerador"  as integer) and  det."glosa_id" = glosas.id and det."glosa_id"=' + "'" + str(elemento) + "'" + ')  where cons."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'"
                    print ("Rips Conulta glosas")
                    curx.execute(comando)

                if (tipoRips == 'Nota Credito'):

                    pass
                    print("Rips Consulta Notas credutos")

                    detalle = 'INSERT INTO rips_ripsconsultas ("notaCredito_id","codPrestador", "numAutorizacion", 	"numDocumentoIdentificacion",  "vrServicio", "valorPagoModerador", "numFEVPagoModerador",consecutivo, "fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id",  "tipoDocumentoIdentificacion_id", "usuarioRegistro_id", "ripsDetalle_id", "itemFactura","ripsTipos_id", "ripsTransaccion_id", ingreso_id, "estadoReg" , "notasCreditoGlosa") SELECT glosas.id, "codPrestador", "numAutorizacion","numDocumentoIdentificacion", "vrServicio", "valorPagoModerador", "numFEVPagoModerador",row_number() OVER(ORDER BY med.id) AS consecutivo , med."fechaRegistro", "codDiagnosticoPrincipal_id", "codDiagnosticoRelacionado_id", "codTecnologiaSalud_id", "conceptoRecaudo_id",  "tipoDocumentoIdentificacion_id", med."usuarioRegistro_id", ' + "'" + str(elementox['id']) + "'" + ' , med."itemFactura",med."ripsTipos_id", ' + "'" + str(transaccionId) + "','"  + str(ingresoId) + "','A'"  + ', gloDetRips."valorAceptado"  FROM rips_ripstransaccion inner join rips_ripsenvios env on(env."sedesClinica_id" = rips_ripstransaccion."sedesClinica_id" and env.id = rips_ripstransaccion."ripsEnvio_id" ) inner join sitios_sedesclinica sed on (sed.id = env."sedesClinica_id" )  inner join rips_ripsconsultas cons on (' + str('1=1') + ')  inner join cartera_notascreditodetallerips notasCreditoDetRips  ON (notasCreditoDetRips."ripsConsultas_id" = cons.id)  inner join cartera_notascreditodetalle  notasCreditoDetalle on (notasCreditoDetalle.id = notasCreditoDetRips."notaCredito_id") inner join cartera_notascredito notasCredito on (notasCredito.id = notasCreditoDetalle."notaCredito_id" and notasCreditoDetallet.factura_id = cast(cons."numFEVPagoModerador"  as integer)) inner join rips_ripsdetalle det on (det."numeroFactura_id" =  cast(cons."numFEVPagoModerador"  as integer) and  det."notaCRedito_id" = notasCredito.id and det."notaCredito_id"=' + "'" + str(elemento) + "'" + ')  where cons."numFEVPagoModerador" = ' + "'" + str(elementoOtro) + "'"
                    print ("Rips Conulta glosas")
                    curx.execute(comando)


                print("ANtes de COPAGOS, etc transaccionId = ", transaccionId)
			
                ## INICIO RUTINA APLICAR PAGOS : COPAGOS-MODERADORA 

                comando = 'SELECT documento_id documento, convenio_id convenio,mod.codigo tipoPago, sum(pagos.valor) valorPago FROM cartera_pagos pagos INNER JOIN  cartera_formaspagos formaPago ON (formaPago.id = pagos."formaPago_id") INNER JOIN rips_ripstipospagomoderador mod ON (mod.id = formaPago."codigoRips_id") WHERE pagos.documento_id = ' + "'" + str(facturaFev.documento_id)  + "'" + ' AND pagos.consec = ' + "'" + str(facturaFev.consecAdmision) + "'" + ' AND pagos.convenio_id = ' + str(facturaFev.convenio_id) +  ' group by documento_id,convenio_id,mod.codigo'
                print("comando = ", comando)
                ...
                for documento, convenio, tipoPago, valorPago in curx.fetchall():

                       aplicando = 0.0
                       detalle = 'SELECT id, "vrServicio" vrServicio FROM rips_ripsprocedimientos where "ripsTransaccion_id" = ' + str(transaccionId)
                       print("detalle = ", detalle)
                       curx.execute(detalle)
                 
                       for id, vrServicio in curx.fetchall():

                             print("vrServiciosProc = ", vrServicio)
                             print("valorPago = ", valorPago)
                             print("tipoPago.id = ", tipoPago)



                             if (aplicando == 0.0 and float(vrServicio) >= float(valorPago)):

                                aplicando=1.0 
                                print("Entre a aplicar un pago a un PROC")   

                                detalle2 = 'UPDATE rips_ripsprocedimientos SET "tipoPagoModerador_id" = ' + str(tipoPago) + ', "valorPagoModerador" = ' + str(valorPago) + ' WHERE id = ' + str(id)
                                print("detalle2 = ", detalle2)
                                curx.execute(detalle2)

                       #aplicando = 0.0
                       detalle = 'SELECT id, "vrServicio" vrServicio FROM rips_ripsmedicamentos where  "ripsTransaccion_id" = ' + str(transaccionId)
                       curx.execute(detalle)

                       for id, vrServicio in curx.fetchall():

                             print("vrServicioMed = ", vrServicio)

                             if (aplicando == 0.0 and vrServicio >= valorPago):

                                aplicando=1.0    

                                detalle2 = 'UPDATE rips_ripsmedicamentos SET "tipoPagoModerador_id" = ' + str(tipoPago) + ', "valorPagoModerador" = ' + str(valorPago) + ' WHERE id = ' + str(id)
                                print("detalle2 actualiza " , detalle2 )
                                curx.execute(detalle2)


                       #aplicando = 0.0
                       detalle = 'SELECT id, "vrServicio" vrServicio FROM rips_ripsOtrosServicios where  "ripsTransaccion_id" = ' + str(transaccionId)
                       curx.execute(detalle)

                       for id, vrServicio in curx.fetchall():

                             print("vrServicioOtrosServ = ", vrServicio)

                             if (aplicando == 0.0 and vrServicio >= valorPago):

                                aplicando=1.0    

                                detalle2 = 'UPDATE rips_ripsotrosservicios SET "tipoPagoModerador_id" = ' + str(tipoPago) + ', "valorPagoModerador" = ' + str(valorPago) + ' WHERE id = ' + str(id)
                                curx.execute(detalle2)



                       #aplicando = 0.0
                       detalle = 'SELECT id, "vrServicio" vrServicio FROM rips_ripsConsultas where  "ripsTransaccion_id" = ' + str(transaccionId)
                       curx.execute(detalle)

                       for id, vrServicio in curx.fetchall():

                             print("vrServicioConsultas = ", vrServicio)

                             if (aplicando == 0.0 and vrServicio >= valorPago):

                                aplicando=1.0    

                                detalle2 = 'UPDATE rips_ripsConsultas SET "tipoPagoModerador_id" = ' + str(tipoPago) + ', "valorPagoModerador" = ' + str(valorPago) + ' WHERE id = ' + str(id)
                                curx.execute(detalle2)



      
                ## FIN RUTINA APLICAR PAGOS : COPAGOS-MODERADORA 
                print ("uff sali de Copagos")
                ...
                if (tipoRips == 'Factura'):
                    ...
                    detalle = 'SELECT generaFacturaJSONBAK1(' + "'" + str(envioRipsId) + "','" + str(elemento) + "'" + ',' + "'" + str('FACTURA') + "'," + str(transaccionId) + ') dato'
            ...
            return JsonResponse({'success': True, 'Mensajes': 'Rips JSON generados satisfactoriamente!'})
...
def BorrarDetalleRips(request):
    ...
    print("aa esto le falta desarrollo por que hay que tener encuenta el tipo : NOTA CREDITO/GLOSA/FACTURA")
    ...
    try:
        with transaction.atomic():
            ...
            RipsOtrosServicios.objects.filter(ripsDetalle_id=envioDetalleRipsId).delete()
            ...
            print("ya borre otros servicios")

            if (envio.ripsTiposNotas_id==2):
                print ("Entere NotaCredito" + str(envioDetalle.notaCredito_id))
                RipsTransaccion.objects.filter(ripsEnvio_id=envio.id, numFactura=envioDetalle.numeroFactura_id , numNota=envioDetalle.notaCredito_id).delete()

            if (envio.ripsTiposNotas_id == 4  ):
                print ("Entere Glosa" + str(envioDetalle.glosa_id))
                RipsTransaccion.objects.filter(ripsEnvio_id=envio.id, numFactura=envioDetalle.numeroFactura_id , numNota=envioDetalle.glosa_id).delete()


            if (envio.ripsTiposNotas_id == 5  ):
                print ("Entere Factura" + str(envioDetalle.notaCredito_id))
                RipsTransaccion.objects.filter(ripsEnvio_id=envio.id, numFactura=envioDetalle.numeroFactura_id).delete()


            print("ya borre RipsTransaccion")
    ...
    print("ya borre transcciones RIPS")
...
def load_dataRipsEnviados(request, data):
    ...
    detalle = 'SELECT env.id,  env."fechaEnvio",  env."fechaRadicacion",  env."fechaRespuesta", env."cantidadFacturas", env."cantidadPasaron", env."cantidadRechazadas",env."ripsEstados_id",  estrips.nombre estadoMinisterio, env."fechaRegistro", env."estadoReg", env."usuarioRegistro_id", env.empresa_id, env."sedesClinica_id" , sed.nombre nombreClinica, emp.nombre nombreEmpresa , pla.nombre nombreRegistra , tiposNotas.nombre tipoNota FROM public.rips_ripsenvios env, sitios_sedesclinica sed, facturacion_empresas emp, planta_planta pla , rips_ripstiposnotas tiposNotas , rips_ripsestados estrips where env."sedesClinica_id" = sed.id and env.empresa_id=emp.id AND pla.id = env."usuarioRegistro_id" AND env."ripsTiposNotas_id" = tiposNotas.id AND estrips.id = env."ripsEstados_id" AND env."sedesClinica_id" =' +  "'" + str(sede) + "' AND (estRips.nombre = " + "'" + str('ENVIADA') + "' or estRips.nombre = '" + str('CON RESPUESTA') + "' or estRips.nombre = '" + str('RADICADO') + "'  or estRips.nombre = '" + str('RECHAZADA') + "')"
    print(detalle)

    curx.execute(detalle)

    for id,  fechaEnvio, fechaRadicacion, fechaRespuesta, cantidadFacturas, cantidadPasaron, cantidadRechazadas, estadoPasoMinisterio, estadoMinisterio ,fechaRegistro, estadoReg, usuarioRegistro_id, empresa_id, sedesClinica_id, nombreClinica, nombreEmpresa, nombreRegistra, tipoNota in curx.fetchall():
        ripsEnviados.append(
            {"model": "rips.ripsEnvios", "pk": id, "fields":
                {'id': id, 'fechaEnvio': fechaEnvio,'fechaRadicacion':fechaRadicacion,  'fechaRespuesta': fechaRespuesta, 'cantidadFacturas': cantidadFacturas,
                 'cantidadPasaron': cantidadPasaron, 'cantidadRechazadas': cantidadRechazadas,
                 'estadoMinisterio': estadoMinisterio, 'estadoMinisterio':estadoMinisterio,  'fechaRegistro': fechaRegistro, 'estadoReg': estadoReg,'usuarioRegistro_id':usuarioRegistro_id, 'empresa_id':empresa_id, 'sedesClinica_id': sedesClinica_id, 'nombreClinica':nombreClinica, 'nombreEmpresa':nombreEmpresa,'nombreRegistra':nombreRegistra, 'tipoNota':tipoNota}})
