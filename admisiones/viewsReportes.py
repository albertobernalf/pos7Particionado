...
from sitios.models import Dependencias, Departamentos, Municipios, SedesClinica
...
class PDFAtencionInicialUrgencias(FPDF):
    ...
    def header(self):
        ...
        self.cell(15, 10, self.ingresoId, 0, 0, 'L')
        self.set_font('Helvetica', 'B', 8)
        ...
        self.ln(3)
        self.set_line_width(0.3)
        self.rect(5.0, 31.0, 120.0, 3.0)  # Coordenadas x, y, ancho, alto
        self.cell(120, 10, historia[0]['nombreEmpresa'], 0, 0, 'L')
        self.rect(130.0, 31.0, 70.0, 3.0)  # Coordenadas x, y, ancho, alto
        self.cell(25, 10, 'Nit: ', 0, 0, 'L')
        self.cell(25, 10, 'X', 0, 0, 'L')
        self.cell(20, 10, historia[0]['nit'], 0, 0, 'L')
        self.rect(200.0, 31.0, 5.0, 3.0)  # Coordenadas x, y, ancho, alto
...
class PDFFormatoFurips(FPDF):
    ...
    def header(self):
        ...
        pdf.ln(3)
        pdf.cell(200, 10, 'REPUBLICA DE COLOMBIA', 0, 0, 'C')
        pdf.set_font('Helvetica', '', 8)
        pdf.ln(3)
        pdf.set_font('Helvetica', 'B', 8)
        pdf.cell(200, 10, 'MINISTERIO DE LA PROTECCION SOCIAL', 0, 0, 'C')
        pdf.set_font('Helvetica', '', 8)
        pdf.ln(3)
        pdf.set_font('Helvetica', 'B', 8)
        pdf.cell(200, 10, 'FORMULARIO UNICO DE RECLAMACIONES DE LAS INSTITUCIONES PRESTADORAS DE SALUD', 0, 0, 'C')
        pdf.set_font('Helvetica', '', 8)
        pdf.ln(3)
        pdf.set_font('Helvetica', 'B', 8)
        pdf.cell(200, 10, 'POR SERVICIOS PRESTADO A VICTIMAS DE EVENTOS CATASTROFICOS Y ACCIDENTES DE TRANSITO', 0, 0, 'C')
        pdf.set_font('Helvetica', '', 8)
        pdf.ln(3)
        pdf.set_font('Helvetica', 'B', 8)
        pdf.cell(200, 10, 'PERSONAS JURIDICAS - FURIPS', 0, 0, 'C')
        pdf.set_font('Helvetica', '', 8)
        pdf.ln(3)
...
def ImprimirAtencionUrgencias(request):
    ...
    pdf.rect(5.0, 50.0, 200.0, 5.0)  # Coordenadas x, y, ancho, alto
   ...
   #pdf.cell(25, 14, 'Numero de Documento de Identificacion', 0, 0, 'L')
    ...
    pdf.rect(5.0, 95.0, 200.0, 20.0)  # Coordenadas x, y, ancho, alto
    pdf.ln(3)
    ...
    pdf.ln(4)
    pdf.rect(5.0, 158.0, 200.0, 30.0)  # Coordenadas x, y, ancho, alto
    ...
    pdf.cell(45, 108, 'Remision', 0, 0, 'L')
...
def ImprimirHojaAdmisionParametro(ingresoId):
    ...
    pdf.cell(230, 1028, 'Manilla de Identificacion#:', 0, 0, 'L')
    ...
    pdf.cell(30, 10, hospitalizacion[0]['ocupacion'], 0, 0, 'L')
    pdf.cell(15, 10, 'Estado Civil:', 0, 0, 'L')
    pdf.cell(30, 10, hospitalizacion[0]['estadoCivil'], 0, 0, 'L')
    pdf.ln(3)
...
def ImprimirHojaAdmision(request):
    ...
    print("Entre ImprimirHojaAdmision1 ", ingresoId)
    #ingresoId = request.POST["ingresoId"]
    ...
    #escribe ='(Ley 1438 del 2011 Art. 143 Según Circular externa 0000033 de 2011 del MINISTERIO DE LA PROTECCION SOCIAL y Resolución 1915 del 2008) , La informacion aquí registrada del evento catalogado como accidente de transito, es declarada bajo la gravedad de juramento por el usuario: con documento de identificación numero:              quien reside en la dirección:, del municipio de: en calidad de paciente y/o acudiente del paciente:___________________________ con documento de identificación numero:_______________, , donde resulto afectado por vehiculo automotor en movimiento.: - Mediante la firma de esta declaración, confirma la veracidad y exactitud de las declaraciones que formulay  , manifestando que nada ha ocultado ,omitido o alterado y se da por enterado que esta declaración constituye para la Compañía prestadora de servcicios en salud información determinante del siniestro , provocándolo intencionalmente, presentándolo ante el asegurador como ocurrido por causas o en circunstancias distintas a las verdaderas, ocultando la cosa asegurada o aumentando fraudulentamente las pérdidas efectivamente sufridas, incurre en el delito de fraude al seguro establecido en el artículo 470, número 10 del código final, o en mi representacion __________________________________________ identificado con ___________________ Declaro que la informacion y/o documentacion aportada y consignada en el presente formato es cierta, veraz y verificable; razón por la cual autorizo su posterior verificacion por parte de la aseguradora y de la misma institucion. Teniendo en cuenta el artículo 9 de la Ley 1581 de 2012 “Por la cual se dictan disposiciones generales para la proteccion de datos personales”, autorizo expresamente a la Clínica Medical S.A.S. a divulgar la informacion aqui reposada tanto internamente como a EPS, aseguradoras, entes de control y demas entidades que la requieran y que esten autorizadas para tal fin, siempre y cuando dicha divulgacion este relacionada con los motivos por los cuales recibí tratamiento en esta Institucion prestadora de salud. De igual Nombre Completo:'
    escribe ='(Ley 1438 del 2011 Art. 143 Según Circular externa 0000033 de 2011 del MINISTERIO DE LA PROTECCION SOCIAL y Resolución 1915 del 2008) '

    print("escribe = " , escribe)
    pdf.multi_cell(w=0, h=3, txt=escribe, align='L')
...
def ImpresionManilla(request):
    ...
    pdf.set_line_width(0.4)
    # Dibuja el borde
    pdf.rect(5.0, 15.0, 170.0, 18.0)  # Coordenadas x, y, ancho, alto

    pdf.set_font('Helvetica', 'B', 8)
    pdf.ln(3)
    pdf.cell(20, 10, 'Nombres:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(25, 10, str(manilla[0]['primerNombre']), 0, 0, 'L')
    pdf.cell(25, 10, str(manilla[0]['segundoNombre']), 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(20, 10, 'Apellidos:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(25, 10, str(manilla[0]['primerApellido']), 0, 0, 'L')
    pdf.cell(25, 10, str(manilla[0]['segundoApellido']), 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(10, 10, 'Riesgo:', 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.ln(3)
    pdf.cell(20, 10, 'Identificacion:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'Fecha Hora de Ingreso:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(15, 10, str(manilla[0]['fechaIngreso']), 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(20, 10, 'Alergias:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
...
def ImprimirAutorizacionesAdm(request):
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.ln(3)
    pdf.cell(100, 10, 'AUTORIZACION:', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
...
def ImprimirFormatoFurips(request):
    # Instantiation of inherited class
    ...
    pdf = PDFFormatoFurips(tipoDocId, documentoId, consec, ingresoId)
    pdf.alias_nb_pages()
    pdf.set_margins(left=10, top=5, right=5)
    pdf.add_page()

    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'I. DATOS DE LA RECLAMACION', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)

    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(160, 10, 'Fecha Radicacion:', 0, 0, 'L')
    pdf.cell(20, 10, 'No Radicado:', 0, 0, 'L')
    pdf.ln(2)
    pdf.cell(160, 10, '', 0, 0, 'L')
    pdf.cell(10, 10, 'GP:', 0, 0, 'L')
    pdf.cell(10, 10, 'GT:', 0, 0, 'L')
    pdf.cell(10, 10, 'GTV:', 0, 0, 'L')
    pdf.ln(2)
    pdf.cell(160, 10, 'No. Radicado anterior:', 0, 0, 'L')
    pdf.cell(20, 10, 'Nro Factura:', 0, 0, 'L')
    pdf.ln(3)

    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'II. DATOS DE LA INSTITUCION PRESTADORA DE SERVICIOS DE SALUD', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)

    ## Datos prestadora de servicios   

    # Datos de la empresa
   
    datosEmpresa = Empresas.objects.get(nombre='CLINICA MEDICAL S.A.S')
    sedeEmpresa = SedesClinica.objects.get(id=ingresoPaciente.sedesClinica_id)
    departamentoEmpresa= Departamentos.objects.get(id=datosEmpresa.departamento_id)
    minicipioEmpresa = Municipios.objects.get(id=datosEmpresa.municipio_id)

    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10, 'Razón Social:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(100, 10, datosEmpresa.nombre, 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10, 'Código de Habilitación:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(100, 10, sedeEmpresa.habilitacion, 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(10, 10, 'Nit:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(100, 10, datosEmpresa.documento, 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10, 'Dirección:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(100, 10, datosEmpresa.direccion, 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10, 'Departamento:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(100, 10, departamentoEmpresa.nombre, 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10, 'Cod:', 0, 0, 'L')
    pdf.cell(100, 10, departamentoEmpresa.codigo, 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10,'Telefono:', 0, 0, 'L')
    pdf.cell(100, 10, datosEmpresa.telefono, 0, 0, 'L')

    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10, 'Municipio:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(100, 10, municipioEmpresa.nombre, 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(80, 10, 'Cod:', 0, 0, 'L')
    pdf.cell(100, 10, municipioEmpresa.codigo, 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.ln(3)

    # Fin Datos de la empresa

    pdf.cell(200, 10, 'III. DATOS DE LA VICTIMA DEL EVENTO CATASTRÓFICO O ACCIDENTE DE TRANSITON', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    curt = miConexiont.cursor()
    comando = 'SELECT usu."primerNombre", usu."segundoNombre", usu."primerApellido" , usu."segundoApellido" , tipDoc.codigo codigoTipoDoc, usu.documento, usu."fechaNacio", usu.genero, usu.direccion, usu.telefono, dep.codigo depCodigo, dep.nombre depNombre, muni.codigo muniCodigo, muni.nombre muniNombre   FROM usuarios_usuarios usu INNER JOIN usuarios_tiposDocumentos tipDoc ON (tipDoc.id = usu."tipoDoc_id")  INNER JOIN sitios_departamentos dep ON (dep.id=usu.departamento_id) INNER JOIN sitios_municipios muni ON (muni.id = usu.municipio_id) WHERE usu.id =' + "'" + str(ingresoPaciente.documento_id) + "'"
    ...
    for primerNombre,segundoNombre,primerApellido, segundoApellido, codigoTipoDoc, documento, fechaNacio, genero, direccion,telefono,depCodigo,depNombre, muniCodigo,  muniNombre    in curt.fetchall():
        datosVictima.append({'primerNombre':primerNombre, 'segundoNombre': segundoNombre, 'primerApellido': primerApellido, 'segundoApellido': segundoApellido,
             'codigoTipoDoc': codigoTipoDoc,'documento':documento,'fechaNacio':fechaNacio, 'genero':genero,'direccion':direccion,'telefono':telefono, 'depCodigo':depCodigo, 'depNombre':depNombre,'muniCodigo':muniCodigo,  'muniNombre':muniNombre   })

    #miConexiont.close()
    print("datosVictima = ", datosVictima)

    pdf.set_font('Helvetica', '', 8)
    pdf.cell(40, 10, str(datosVictima[0]['primerNombre']), 0, 0, 'L')
    pdf.cell(40, 10, str(datosVictima[0]['segundoNombre']), 0, 0, 'L')
    pdf.cell(40, 10, str(datosVictima[0]['primerApellido']), 0, 0, 'L')
    pdf.cell(40, 10, str(datosVictima[0]['segundoApellido']), 0, 0, 'L')
    pdf.ln(3)
    pdf.cell(40, 10, '1er. Apellido', 0, 0, 'C')
    pdf.cell(40, 10, '2do. Apellido', 0, 0, 'C')
    pdf.cell(40, 10, '1er. Nombre', 0, 0, 'C')
    pdf.cell(40, 10, '2do. Nombre', 0, 0, 'C')

    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(150, 10, 'Tipo de documento:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(10, 10, str(datosVictima[0]['codigoTipoDoc']), 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(20, 10, 'Nro. Documento:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(30, 10, str(datosVictima[0]['documento']), 0, 0, 'L')
    pdf.ln(3)

    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(150, 10, 'Fecha de Nacimiento:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(10, 10, str(datosVictima[0]['fechaNacio']), 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(20, 10, 'Sexo:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(30, 10, str(datosVictima[0]['genero']), 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(20, 10, 'Fecha de Fallecimiento:', 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(150, 10, 'Dirección Residencia:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(10, 10, str(datosVictima[0]['direccion']), 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(50, 10, 'Departamento:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(80, 10, str(datosVictima[0]['depNombre']), 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(5, 10, 'Cod:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(10, 10, str(datosVictima[0]['depCodigo']), 0, 0, 'L')
    pdf.cell(20, 10, 'Telefono:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(20, 10, str(datosVictima[0]['telefono']), 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(50, 10, 'Municipio:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(80, 10, str(datosVictima[0]['muniNombre']), 0, 0, 'L')
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(5, 10, 'Cod:', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.cell(10, 10, str(datosVictima[0]['muniCodigo']), 0, 0, 'L')

    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(50, 10, 'Condición del Accidentado:', 0, 0, 'L')
    pdf.cell(30, 10, 'Conductor:', 0, 0, 'L')
    pdf.cell(30, 10, 'Peatón:', 0, 0, 'L')
    pdf.cell(30, 10, 'Ocupante:', 0, 0, 'L')
    pdf.cell(30, 10, 'Ciclista:', 0, 0, 'L')

    ## Fin Datos de la victima

    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'IV. DATOS DEL SITIO DONDE OCURRIÓ EL EVENTO CATASTRÓFICO O EL ACCIDENTE DE TRANSITO', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)


    ## Datos donde ocurrio el evento catastrofico
    ...
    #for     in curt.fetchall():
    #    datosEvento.append({ })

    #miConexiont.close()
    print("datosEvento = ", datosEvento)

    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(50, 10, 'NATURALEZA DEL EVENTO:', 0, 0, 'L')
    pdf.ln(3)
    pdf.cell(30, 10, 'Accidente de Transito:', 0, 0, 'L')
    pdf.ln(3)
    pdf.cell(30, 10, 'Sismo', 0, 0, 'L')
    pdf.cell(30, 10, 'Maremoto', 0, 0, 'L')
    pdf.cell(30, 10, 'Erupciones Volcanicas', 0, 0, 'L')
    pdf.cell(30, 10, 'Huracán', 0, 0, 'L')

    pdf.ln(3)
    pdf.cell(30, 10, 'Inundación', 0, 0, 'L')
    pdf.cell(30, 10, 'Avalancha', 0, 0, 'L')
    pdf.cell(30, 10, 'Deslizamiento de Tierra', 0, 0, 'L')
    pdf.cell(30, 10, 'Incendio Natural', 0, 0, 'L')

    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(50, 10, 'Terroristas:', 0, 0, 'L')
    pdf.cell(30, 10, 'Explosión Terrorista', 0, 0, 'L')
    pdf.cell(30, 10, 'Masacre', 0, 0, 'L')
    pdf.cell(30, 10, 'Mina Antipersonal', 0, 0, 'L')
    pdf.cell(30, 10, 'Combate', 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'Incendio Terrorista', 0, 0, 'L')
    pdf.cell(30, 10, 'Ataque a Municipios', 0, 0, 'L')
    pdf.cell(30, 10, 'Desplazados', 0, 0, 'L')
    pdf.cell(30, 10, 'Rayo', 0, 0, 'L')
    pdf.cell(30, 10, 'Vendaval', 0, 0, 'L')
    pdf.cell(30, 10, 'Tornado', 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'OTROS', 0, 0, 'L')
    pdf.cell(180, 10, 'Cual?', 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'Dirección de la Ocurrencia', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['direccionOcurrencia']), 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'Fecha Evento/Accidente', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['fechaOcurrencia']), 0, 0, 'L')
    pdf.cell(30, 10, 'Hora', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['horaOcurrencia']), 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'Departamento', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['deptoNombre']), 0, 0, 'L')
    pdf.cell(30, 10, 'Cod', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['deptoCodigo']), 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'Municipio', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['muniNombre']), 0, 0, 'L')
    pdf.cell(30, 10, 'Cod', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['miniCodigo']), 0, 0, 'L')
    pdf.cell(30, 10, 'Zona', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['zona']), 0, 0, 'L')
    pdf.ln(3)
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(30, 10, 'Descripción Breve del Evento Catastrófico o Accidente de Transito (Enuncie las principales caracteristicas del evento / accidento', 0, 0, 'L')
    pdf.cell(10, 10, str(datosEvento[0]['descripcion']), 0, 0, 'L')


    ## FIN Datos donde ocurrio el evento catastrofico

    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'V. DATOS DEL VEHICULO DEL ACCIDENTE DE TRANSITO', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    #miConexiont.close()
    print("datosVehiculo = ", datosVehiculo)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'VI. DATOS RELACIONADOS CON LA ATENCION', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'VII. DATOS DEL PROPIETARIO DEL VEHICULO', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'VIII. DATOS DEL CONDUCTOR DEL VEHÍCULO', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'IX. DATOS DE REFERENCIA', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'X. AMPARO DE TRANSPORTE Y MOVILIZACION DE LA VICTIMA', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'XI. DATOS DE LA ATENCIÓN MEDICA DELA VICTIMA COMO PRUEBA DEL ACCIDENTE O EVENTO', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'XII. DATOS DEL MEDICO O PROFESIONAL DE LA SALUD TRATANTE', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'XIII. AMPAROS QUE RECLAMA', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'XIV. DECLARACIONES DE LA INSTITUCION PRESTADORA DE SERVICIOS DE SALUD', 0, 0, 'C')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)


    pdf.set_font('Helvetica', 'B', 8)
    texto = 'Como representante legal o Gerente de la Institucion Prestadora de Salud, declaro bajo la gravedad de juramento que toda la informacion contenida en este formulario es cierta y podra ser verificada por la Direccion General de Financiamiento del Ministerio de la Proteccion Social, por el Administrador Fiduciario del Fondo de Solidaridad y Garantia Fosyga, por la Superintendencia Nacional de Salud o la Contraloria General de la Republica con la IPS y las Aseguradoras, de no ser asi, acepto todas las consecuencias legales que produzca esta situacion.'

    pdf.multi_cell(w=0, h=4, txt=texto, border=0, align='J', fill=False)

    pdf.set_font('Helvetica', 'B', 8)
    pdf.cell(200, 10, 'Manifestación de servicios habilitados: SID', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)
    ...
    pdf.cell(50, 10, 'FIRMA DEL REPRESENTANTE LEGAL, GERENTE O SU DELEGADO', 0, 0, 'L')
    pdf.set_font('Helvetica', '', 8)
    pdf.ln(3)

    carpeta = 'C:\EntornosPython\Pos7Particionado/vulner/JSONCLINICA\Furips/'
    print("carpeta = ", carpeta)

    archivo = carpeta + '' + str(pacienteId.documento) + '_' + 'Furips.pdf'
    print("archivo =", archivo)
    ...
    return JsonResponse({'success': True, 'message': 'Furips impreso!'})
