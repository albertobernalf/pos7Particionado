...
from cirugia.models import CirugiasProcedimientos, Cirugias
...
def ActualizarAutorizacionDetalle(request):
    ...
    if (datosAut.observaciones == "AUTORIZACION HOSPITALARIA" and float(estadoAutorizacionAutorizado.id) == float(estadoAutorizacion)):
        ...
        try:
            ...
            curt=miConexiont.cursor()
            comando = 'UPDATE admisiones_ingresos set "autorizacionesHospitalDetalle_id" = ' + "'" + str(autorizacionDetalleId) + "'" + ' WHERE id = ' + "'" + str(datosAut.ingreso_id) + "'"
            ...
            detalle = 'UPDATE autorizaciones_autorizaciones SET  "estadoAutorizacion_id" =   ' + "'" + str(estadoAutorizacion) + "'," + ' "fechaRegistro" = ' + "'" + str(fechaRegistro) + "'" + ' WHERE id = ' + "'" + str(datosAut.id) + "'"
            print("detalle = ", detalle)
            curt.execute(detalle)
    ...
    if (datosAut.observaciones == "AUTORIZACION DE CIRUGIA" and estadoAutorizacionAutorizado.id == int(estadoAutorizacion)):
        ...
        cirugiasProcId = CirugiasProcedimientos.objects.get(cirugia_id=cirugiaId.id, cups_id=examenId)
        ...
        try:
            ...
            miConexiont = psycopg2.connect(host="192.168.133.128", database="vulner7Particionado", port="5432",
                                           user="postgres", password="123456")
            curt = miConexiont.cursor()
            comando = 'UPDATE cirugia_cirugiasprocedimientos  set "autorizacionDetalle_id" = ' + "'" + str(autorizacionDetalleId) + "'" + ' WHERE id = ' + "'" + str(cirugiasProcId.id) + "'"

            print("comando = ", comando)
            curt.execute(comando)

            detalle = 'UPDATE autorizaciones_autorizacionesdetalle SET  "estadoAutorizacion_id" =   ' + "'" + str(
                estadoAutorizacion) + "'," + ' "numeroAutorizacion" = ' + "'" + str(
                numeroAutorizacion) + "'," + ' "fechaRegistro" = ' + "'" + str(
                fechaRegistro) + "'" + ' WHERE id = ' + "'" + str(autorizacionDetalleId) + "'"
            print("detalle = ", detalle)
            curt.execute(detalle)

            detalle = 'UPDATE autorizaciones_autorizaciones SET  "estadoAutorizacion_id" =   ' + "'" + str(
                estadoAutorizacion) + "'," + ' "fechaRegistro" = ' + "'" + str(
                fechaRegistro) + "'" + ' WHERE id = ' + "'" + str(datosAut.id) + "'"
            print("detalle = ", detalle)
            curt.execute(detalle)
            ...
            return JsonResponse({'success': True, 'Mensajes': 'Autorizacion de cirugia realizada con exito'})
...
def LeerDetalleAutorizacion(request):
    ...
    for tipoTipoExamen,id, cantidadSolicitada, cantidadAutorizada, fechaRegistro, estadoReg, autorizaciones_id, usuarioRegistro_id, tipNombre, exaNombre, examenes_id,  valorAutorizado,	valorSolicitado, tiposExamen_id, tipoSuministro_id, estadoAutorizacion_id, numeroAutorizacion, estadoNombre , convenioId, mipres in curx.fetchall():
        autorizacionDetalle.append(
            {"model": "autorizaciones_autorizacionesdetalle", "pk": id, "fields":
                {'tipoTipoExamen':tipoTipoExamen, 'id':id, 'cantidadSolicitada':cantidadSolicitada,'cantidadAutorizada':cantidadAutorizada,'fechaRegistro':fechaRegistro,'estadoReg':estadoReg,
                 ...
                 'valorSolicitado':valorSolicitado,'tiposExamen_id':tiposExamen_id,'tipoSuministro_id':tipoSuministro_id,'estadoAutorizacion_id':estadoAutorizacion_id,'numeroAutorizacion':id,'numeroAutorizacion':numeroAutorizacion,'estadoNombre':estadoNombre, 'convenio_id':convenioId,'mipres':mipres}})
