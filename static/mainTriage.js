		        ...
		        { data: "fields.triage"},
{
	  ...
	  initComplete: function(settings, json) {
		...
		var primeraFila = $('#tablaDatosTriage tbody tr:eq(0) input[type="radio"]').prop('checked', true);
             // $(primeraFila).click();
...
$('#tablaDatosTriage tbody').on('click', '.miTriage', function() {
	...
	// Aqui es la creacion de la Admision para el triage
      ...
      $.ajax({
		...
		data: {'tiposDoc':dato3.tipoDoc,'documento':dato3.Documento,'sede':sede, 'username':username,'triageId':dato3.id},
		success: function (response_data) {
   			     alert("entre DATOS MODAL ADMISION DESDE TRIAGE  de tipoDoc y documento  = " + JSON.stringify(response_data));
			...
			// alert("voy para SubServicios  = " + response_data['SubServicios']);
				...
				$('#numManilla').val('0');
				$('#empresasT').val(response_data['EmpresasT']);
				$('#conveniosT').val(response_data['ConveniosT']);
				$('#nombreEmpresasT').val(response_data['NombreEmpresasT']);
				$('#nombreConveniosT').val(response_data['NombreConveniosT']);
