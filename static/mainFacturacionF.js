                   ...
                   },
{
	  ...
	  initComplete: function(settings, json) {
		...
		var primeraFila = $('#tablaLiquidacions tbody tr:eq(0) input[type="radio"]').prop('checked', true);
             $(primeraFila).click();
...
$.ajax({
 ...
 data : {ingresoId:ingresoId},
	...
	$.ajax({
		...
		document.getElementById("textoXml").value = data.contenido_Xml;
		document.getElementById("textoJson").value = data.contenido_Json;
		document.getElementById("respuestaXml").value = data.contenido_XmlRta;
		...
		$('#RvalorAPagarLetras').val(data.valorAPagarLetras);
			...
			data2['flag'] = 'FACTURACION'
		        ...
		        data2['facturacionId'] = post_id;
                        ...
                        alert("Voy a traer el detalle de la Factura No = " + post_id); 
...
function AdicionarLiquidacion()
{
 	...
 	var liquidacionId = document.getElementById("liquidacionId").value;
        ...
        alert("Voy a guardarliquidacionDetalle");
		...
		$.ajax({
			...
			'valorTotal':valorTotal,'observaciones':observaciones,
	           ...
	           dataType : 'json',
	  		success: function (data) {
		        alert("llegue de liquidacionDetalle");
...
function LeerJson()
{
	...
	if (data.success == true)
			...
			document.getElementById("textoJson").value = data.contenido_Json;
			document.getElementById("textoXml").value = data.contenido_Xml;
			document.getElementById("textoRtaXml").value = data.contenido_XmlRta;
...
function GuardarRespuestaXml2()
{

alert("Entre GuardarRespuestaXml2");

 	var facturaId = document.getElementById("facturaJson").innerHTML;
	

 	var respuestaXml = document.getElementById("respuestaXml").value;


		$.ajax({
	           url: '/guardarRespuestaXml/',
	            data : {'facturaId':facturaId,'respuestaXml':respuestaXml},
	           type: 'POST',
	           dataType : 'json',
	  		success: function (data) {
