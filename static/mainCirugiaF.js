  ...
  lengthMenu: [2, 4, 15],
...

	{
	  "render": function ( data, type, row ) {
	     ...
	     btn = btn + " <button class='miConvenioCirugia btn-primary ' style='width:15px;height:15px;accent-color: purple;border-color: purple;background-color: green;'  data-pk='" + row.pk + "'>" + '<i class="fa-duotone fa-regular fa-thumbs-up"></i>' + "</button>";
...
//		},
		...
		var primeraFila = $('#tablaProgramacionCirugia tbody tr:eq(0) input[type="radio"]').prop('checked', true);
           //  $(primeraFila).click();
  ...
  lengthMenu: [2, 4, 15],
	...
	},
	{
	  "render": function ( data, type, row ) {
	     ...
	     btn = btn + " <button class='miSelConvenio btn-primary ' style='width:15px;height:15px;accent-color: purple;border-color: purple;background-color: green;'  data-pk='" + row.pk + "'>" + '<i class="fa-duotone fa-regular fa-thumbs-up"></i>' + "</button>";
...
//		},
 ...
 dom: "<'row mb-0'<'col-sm-6'f><'col-sm-4'><'col-sm-2'B>>" + // B = Botones a la izquierda, f = filtro a la derecha
  ...
  lengthMenu: [2, 4, 15],
	...
	alert("Acabo de entrar disponibilidadsalasCitrugia");
  ...
  lengthMenu: [2, 4, 15],
	 ...
	 pageLength: 3,
	  ...
	  language: {
           ...
           ajax: {
                 url:"/load_dataDispoSalasCirugia/" +  data,
                 type: "POST",
                 dataSrc: ""
            },
            columns: [
...
const initDataTableProgramacionCirugia = async () => {
	if  (dataTableProgramacionCirugiaInitialized)  {
	 ...
	 //   dataTableSolicitudCirugiaInitialized = true;
       ...
       alert("Carga Programacion de ciru");

        arrancaCirugia(2,data);
	    dataTableProgramacionCirugiaInitialized = true;

       alert("ya Cargue Programacion de ciru");
...
$('#tablaProgramacionCirugia tbody').on('click', '.miConvenioCirugia', function() {

	var post_id = $(this).data('pk');
	var cirugiaId = post_id;

	alert("Entre Crear Convenio a cirugia" + post_id);

	$.ajax({

	        url: "/BuscarConveniosDeCirugia/",
                data: {'cirugiaId':cirugiaId},
                type: "POST",
                dataType: 'json',
                success: function (info) {

            $('#postFormConveniosCirugia').trigger("reset");
     	    // document.getElementById("programacionIdParaEstado").value = post_id;
            $('#modelHeadingConveniosCirugia').html("Seleccionar el convenio a cirugia");
            $('#crearModelConveniosCirugia').modal('show');      
...
function GuardarConvenioCirugia()
{
		 alert("ENTRE GuardarConvenioCirugia");
	     var post_id = $(this).data('pk');
	
	  var cirugia = post_id;
	  var convenioId = document.getElementById("convenioCirugiaProgramacion").value;

	 alert("cirugiaId : " + cirugiaId );
     
	$.ajax({

	        url: "/CrearConveniosDeCirugia/",
                data: {'cirugiaId':cirugiaId,'convenioId':convenioId},
                dataType: 'json',
                success: function (info) {
		...
		var data =  {}   ;
	 	...
	 	var sede = document.getElementById("sede").value;
            ...
            $('#crearModelConveniosCirugia').modal('hide');      	
...
$('#tablaProgramacionCirugia tbody').on('click', '.miEditaProgramacionCirugia', function() {
	...
	document.getElementById("usernameProgramacionCirugia_id").value = username_id;
		...
		// Participantes solicitud e informe
	  		  ...
	  		  // var dato = JSON.parse(info[0]);


             //var dato1 = JSON.parse(info[0]);

                     const $id6 = document.querySelector("#conveniosProg");

 	      		     $("#conveniosProg").empty();


	                 $.each(info[0].ConveniosPaciente, function(key,value) {
	                 ...
	                 $.each(info[0].procedParticipantes, function(key,value) {
                                    options +='<option value="' + value.id + '">' + value.nombre + '</option>';
                                    option = document.createElement("option");
                                    option.value = value.id;
                                    option.text = value.nombre;
	  ...
	  // var dato = JSON.parse(info);
		 ...
		 $('#conveniosProg').val(info[0].fields.convenio_id);
		// fin combos informe
...
$('#tablaSolicitudCirugia tbody').on('click', '.miSelConvenio', function() {

		 alert("ENTREseleccionar convenio");

	     var post_id = $(this).data('pk');
	   var cirugiaIdModalMaterial =   post_id;
		var cirugiaId = post_id;

            $('#postFormConveniosDeCirugia').trigger("reset");
	...
	alert("llegue con estop = " + JSON.stringify(data2[0]));
	            ...
	            const $id5 = document.querySelector("#conveniosDeCirugia");
 	      	$("#conveniosDeCirugia").empty();
            ...
            $('#modelHeadingConveniosDeCirugia').html("Convenios");
                alert("voy a abrir la ventana");
            $('#crearModelConveniosDeCirugia').modal('show');

                },
         error: function (data2) {
	   			    	document.getElementById("mensajesErrorModalConveniosDeCirugia").value =   data.responseText;
...
$('#tablaSolicitudCirugia tbody').on('click', '.miSolicitudCirugia2', function() {
	...
	document.getElementById("cirugiaProcedId").value = post_id;
...
function CrearConveniosDeCirugia()
{
		var row = $(this).closest('tr'); // Encuentra la fila


	var sede = document.getElementById("sede").value;
	cirugiaId = document.getElementById("cirugiaId2").value;
	convenioId = document.getElementById("conveniosDeCirugia").value;

	 alert("CrearConveniosDeCirugia = " + cirugiaId);
	 alert("convenioId = " + convenioId);
            ...
            $.ajax({
                ...
                success: function (data2) {
                  ...
                  $('#postFormCrearConveniosDeCirugia').trigger("reset");
	    ...
	    dataTableSolicitudCirugiaInitialized = true;
 		 ...
 		 $('#crearModelConveniosDeCirugia').modal('hide');
			 document.getElementById("mensajesModalConveniosDeCirugia").value = data2.Mensajes;
