...
from facturacion.models import ConveniosPacienteIngresos, Empresas
...
def admisionTriageModal(request):
    ...
    triage=Triage.objects.get(id=triageId)
    print("triage convenio = ", triage.convenio_id)
    print("triage empresa = ", triage.empresa_id)

    empresaId = Empresas.objects.get(id=triage.empresa_id)
    print("si_01")
    convenioId = Convenios.objects.get(id=triage.convenio_id)

    print("si_02")

    response_data['EmpresasT'] = triage.empresa_id
    response_data['ConveniosT'] = triage.convenio_id
    response_data['NombreEmpresasT'] = empresaId.nombre
    response_data['NombreConveniosT'] = convenioId.nombre
    print("si_04")

    # Combo Empresas la que tiene el Triage actual
...
def guardarAdmisionTriage(request):
    ...
    if request.method == 'POST':
       ...
       # documento = request.POST['documento']
        ...
        if (autHospitalizacion== ''):
           autHospitalizacion='null'
